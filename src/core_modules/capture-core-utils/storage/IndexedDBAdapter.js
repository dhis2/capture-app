...
class IndexedDBAdapter {
  ...
  static isSupported() {
    return (
      IndexedDBAdapter.indexedDB && IndexedDBAdapter.iDBKeyRange && IndexedDBAdapter.iDBTransaction
    );
  }
  ...
  static get(store, key, db, keyPath) {
    return new Promise((resolve, reject) => {
      ...
      try {
        ...
        tx.onabort = () => {
          reject(
            errorCreator(IndexedDBAdapter.errorMessages.GET_FAILED)({
              aborted: true,
              error: catchError,
            }),
          );
  ...
  static facilitateDowngradeIfApplicable(name, version, keyPath, onBeforeUpgrade) {
    ...
    return new Promise((resolve, reject) => {
      ...
      preCheckRequest.onerror = (error) => {
        log.error(
          errorCreator(IndexedDBAdapter.errorMessages.OPEN_FAILED)({
            error,
          }),
        );
        reject(IndexedDBAdapter.errorMessages.OPEN_FAILED);
  ...
  async open(onBeforeUpgrade, onAfterUpgrade, onCreateObjectStore) {
    const isDowngrade = await IndexedDBAdapter.facilitateDowngradeIfApplicable(
      this.name,
      this.version,
      this.keyPath,
      onBeforeUpgrade,
    ...
    await new Promise((resolve, reject) => {
      ...
      request.onblocked = (error) => {
        wasBlocked = true;
        reject(
          errorCreator(IndexedDBAdapter.errorMessages.OPEN_BLOCKED)({
            adapter: this,
            error,
          }),
        );
        ...
        const insertItem = () => {
          if (storedCount < dataArray.length) {
            ...
            if (!isDefined(storeObject) || !isDefined(storeObject[this.keyPath])) {
              abortTx(
                errorCreator(IndexedDBAdapter.errorMessages.INVALID_STORAGE_OBJECT)({
                  adapter: this,
                }),
              );
            }
      ...
      try {
        ...
        tx.oncomplete = () => {
          if (isAborted && isAborted()) {
            reject(
              errorCreator(IndexedDBAdapter.errorMessages.GET_ALL_FAILED)({
                adapter: this,
                aborted: true,
                error: catchError,
              }),
        ...
        tx.onerror = (error) => {
          ...
          reject(
            errorCreator(IndexedDBAdapter.errorMessages.GET_ALL_FAILED)({ adapter: this, error }),
          );
        };
  ...
  getKeys(store) {
    return new Promise((resolve, reject) => {
      ...
      try {
        ...
        tx.onerror = (error) => {
          reject(
            errorCreator(IndexedDBAdapter.errorMessages.GET_KEYS_FAILED)({ adapter: this, error }),
          );
        };
        tx.onabort = () => {
          reject(
            errorCreator(IndexedDBAdapter.errorMessages.GET_KEYS_FAILED)({
              adapter: this,
              aborted: true,
              error: catchError,
            }),
  ...
  remove(store, keys) {
    return new Promise((resolve, reject) => {
      ...
      try {
        ...
        tx.onerror = (error) => {
          reject(
            errorCreator(IndexedDBAdapter.errorMessages.REMOVE_FAILED)({ adapter: this, error }),
          );
        };
        tx.onabort = () => {
          reject(
            errorCreator(IndexedDBAdapter.errorMessages.REMOVE_FAILED)({
              adapter: this,
              aborted: true,
              error: catchError,
            }),
  ...
  removeAll(store) {
    return new Promise((resolve, reject) => {
      ...
      try {
        ...
        tx.onerror = (error) => {
          reject(
            errorCreator(IndexedDBAdapter.errorMessages.REMOVE_ALL_FAILED)({
              adapter: this,
              error,
            }),
          );
        ...
        reject(
          errorCreator(IndexedDBAdapter.errorMessages.REMOVE_ALL_FAILED)({ adapter: this, error }),
        );
      }
    });
  }
  ...
  contains(store, key) {
    return new Promise((resolve, reject) => {
      ...
      try {
        ...
        tx.onerror = (error) => {
          reject(
            errorCreator(IndexedDBAdapter.errorMessages.CONTAINS_FAILED)({ adapter: this, error }),
          );
        };
        tx.onabort = () => {
          reject(
            errorCreator(IndexedDBAdapter.errorMessages.CONTAINS_FAILED)({
              adapter: this,
              aborted: true,
              error: catchError,
            }),
  ...
  count(store, options) {
    return new Promise((resolve, reject) => {
      ...
      try {
        ...
        tx.onerror = (error) => {
          reject(
            errorCreator(IndexedDBAdapter.errorMessages.COUNT_FAILED)({ adapter: this, error }),
          );
        };
        tx.onabort = () => {
          reject(
            errorCreator(IndexedDBAdapter.errorMessages.COUNT_FAILED)({
              adapter: this,
              aborted: true,
              error: catchError,
            }),
        ...
        const objectStore = tx.objectStore(store);
        const request = onIDBGetRequest ? onIDBGetRequest(objectStore) : objectStore.count(query);
  ...
  close() {
    return new Promise((resolve, reject) => {
      ...
      try {
        ...
        reject(
          errorCreator(IndexedDBAdapter.errorMessages.CLOSE_FAILED)({
            adapter: this,
            error,
          }),
        );
  ...
  async destroy() {
    await new Promise((resolve, reject) => {
      this.close()
        ...
        .catch((error) => {
          reject(
            errorCreator(IndexedDBAdapter.errorMessages.CLOSE_FAILED_FROM_DESTROY)({
              adapter: this,
              error,
            }),
          );
