...
export default class StorageController {
  ...
  getOpenStatusError() {
    return !this.adapter.isOpen()
      ? errorCreator(StorageController.errorMessages.STORAGE_NOT_OPEN)({
  ...
  throwIfStoreNotFound(store, caller) {
    if (!store || !this.adapter.objectStoreNames.includes(store)) {
      throw Error(
        errorCreator(StorageController.errorMessages.INVALID_OBJECTSTORE)({
  ...
  throwIfDataObjectError = (dataObject) => {
    if (!dataObject || !dataObject[this.adapter.keyPath]) {
      throw Error(
        errorCreator(StorageController.errorMessages.INVALID_STORAGE_OBJECT)({
          adapter: this.adapter,
        }),
      );
    }
  ...
  throwIfDataArrayError(dataArray) {
    if (!dataArray) {
      throw Error(
        errorCreator(StorageController.errorMessages.INVALID_STORAGE_ARRAY)({
          adapter: this.adapter,
        }),
      );
    }

    dataArray.forEach(this.throwIfDataObjectError);
  }
  ...
  async open(...args) {
    if (this.adapter.isOpen()) {
      throw new Error(
        errorCreator(StorageController.errorMessages.STORAGE_ALREADY_OPEN)({
          adapter: this.adapter,
        }),
      );
    }
  ...
  async get(store, key) {
    ...
    if (!key) {
      throw Error(
        errorCreator(StorageController.errorMessages.MISSING_KEY)({
          adapter: this.adapter,
          key,
          method: 'get',
        }),
