...
class DomLocalStorageAdapter {
  ...
  open(onBeforeUpgrade, onAfterUpgrade) {
    this.indexer = this.objectStoreNames.reduce((accIndexers, store) => {
  ...
  set(store, dataObject) {
    return new Promise((resolve, reject) => {
      try {
        ...
        reject(
          errorCreator(DomLocalStorageAdapter.errorMessages.SET_FAILED)({ adapter: this, error }),
        );
      }
    });
  }
  ...
  count(store, key) {
    if (key) {
      Promise.reject(
        errorCreator(DomLocalStorageAdapter.errorMessages.KEY_BASED_COUNT_IS_NOT_SUPPORTED)({
          adapter: this,
        }),
      );
    }
