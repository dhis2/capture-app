...
function loadSearchOrgUnitRootsAsync() {
  ...
  if (
    // $FlowFixMe[prop-missing] automated comment
    !currentUser.teiSearchOrganisationUnits ||
    currentUser.teiSearchOrganisationUnits.length === 0
  ) {
...
async function getOrgUnitCaptureRootsAction() {
  ...
  const captureRootsForStore = captureRoots.map((unit) => ({
    id: unit.id,
    path: unit.path,
...
async function getOrgUnitSearchRootsAction() {
  ...
  const searchRootsForStore = searchRoots.map((unit) => ({
    id: unit.id,
    path: unit.path,
    displayName: unit.displayName,
  }));
...
export default async function getOrgUnitRootsActions() {
  ...
  return [captureRootsAction, searchRootsAction];
}
