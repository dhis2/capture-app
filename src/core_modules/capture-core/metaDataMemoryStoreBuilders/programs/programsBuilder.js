...
import { Program, TrackedEntityType } from '../../metaData';
...
function getProgramRulesVariables(
  storageController: StorageController,
  storeName: string,
): Promise<Array<Object>> {
  return storageController.getAll(storeName);
...
function getCategories(
  ...
  return storageController.getAll(storeName).then((categoryArray) =>
    categoryArray.reduce((accCategories, category) => {
...
async function getBuilderPrerequisites() {
  ...
  const cachedProgramRulesVariables = getProgramRulesVariables(
    storageController,
    stores.PROGRAM_RULES_VARIABLES,
  );
  const cachedProgramRules = getProgramRules(storageController, stores.PROGRAM_RULES);
  const cachedProgramIndicatorsPromise = getProgramIndicators(
    storageController,
    stores.PROGRAM_INDICATORS,
  );
  const cachedRelationshipTypesPromise = getRelationshipTypes(
    storageController,
...
function addProgramVariables(d2ProgramRulesVariables: Array<ProgramRuleVariable>) {
  const rulesVariablesByProgram = d2ProgramRulesVariables.reduce(
    (accRulesVariablesByProgram, d2RuleVariable) => {
      const ruleVariableProgramId = d2RuleVariable.programId;
      accRulesVariablesByProgram[ruleVariableProgramId] =
        accRulesVariablesByProgram[ruleVariableProgramId] || [];
    ...
    if (program) {
      ...
      const rulesByStage = programRules
        ...
        .reduce((accRulesByStage, programRule) => {
          accRulesByStage[programRule.programStageId] =
            accRulesByStage[programRule.programStageId] || [];
...
function addRulesAndVariablesFromProgramIndicators(
  cachedProgramIndicators: Array<CachedProgramIndicator>,
) {
  const indicatorsByProgram = cachedProgramIndicators.reduce(
    (accIndicatorsByProgram, indicator) => {
  ...
  Object.keys(indicatorsByProgram).forEach((programKey) => {
    ...
    const program = programCollection.get(programKey);
    if (program) {
      const { rules, variables } = getRulesAndVariablesFromProgramIndicators(
        indicators,
        programKey,
      );
...
function postProcessPrograms(
  ...
  sortPrograms(programs).forEach((program) => {
