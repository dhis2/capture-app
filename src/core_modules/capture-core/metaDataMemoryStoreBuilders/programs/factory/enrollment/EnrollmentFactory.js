...
import type { TrackedEntityType, SearchGroup } from '../../../../metaData';
...
class EnrollmentFactory {
  ...
  async _buildSection(
    ...
    if (
      (!cachedProgramTrackedEntityAttributes || cachedProgramTrackedEntityAttributes.length <= 0) &&
      !featureTypeField
    ) {
      return null;
    }
  ...
  async _buildEnrollmentForm(cachedProgram: CachedProgram) {
    ...
    let section = await this._buildSection(
  ...
  _buildInputSearchGroupFoundation(cachedProgram: CachedProgram, searchGroup: SearchGroup) {
    const programTeiAttributes = cachedProgram.programTrackedEntityAttributes || [];
    const teiAttributesAsObject = programTeiAttributes.reduce(
      (accTeiAttributes, programTeiAttribute) => {
        ...
        if (!teiAttribute) {
          log.error(
            errorCreator('could not retrieve tei attribute')({
              programTeiAttribute,
            }),
          );
        } else {
  ...
  async build(cachedProgram: CachedProgram, programSearchGroups: Array<SearchGroup> = []) {
