...
class ProgramFactory {
  ...
  constructor(
    ...
    this.searchGroupFactory = new SearchGroupFactory(cachedTrackedEntityAttributes, locale);
  ...
  _buildCategories(cachedProgramCategories: Array<ProgramCachedCategory>): Map<string, Category> {
    return new Map(
  ...
  _buildCategoryCombination(cachedCategoryCombination: ?ProgramCachedCategoryCombo) {
    if (
      !(
  ...
  async build(cachedProgram: CachedProgram) {
    ...
    if (cachedProgram.programType === 'WITHOUT_REGISTRATION') {
      ...
      // $FlowFixMe
      await cachedProgram.programStages.asyncForEach(
        async (cachedProgramStage: CachedProgramStage) => {
