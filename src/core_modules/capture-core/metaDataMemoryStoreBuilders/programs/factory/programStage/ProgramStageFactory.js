...
class ProgramStageFactory {
  ...
  constructor(
    ...
    this.locale = locale;
    this.relationshipTypesFactory = new RelationshipTypesFactory(cachedRelationshipTypes);
    this.dataElementFactory = new DataElementFactory(cachedOptionSets, locale);
  }
  ...
  async _buildSection(
    ...
    if (sectionSpecs.dataElements) {
      // $FlowFixMe
      await sectionSpecs.dataElements.asyncForEach(
        async (sectionDataElement: CachedSectionDataElements) => {
          ...
          if (!cachedProgramStageDataElement) {
            log.error(
              errorCreator('could not find programStageDataElement')({ sectionDataElement }),
            );
            return;
          }
  ...
  async build(cachedProgramStage: CachedProgramStage, programId: string) {
    const stage = new ProgramStage((_stage) => {
      ...
      _stage.stageForm = new RenderFoundation((_form) => {
        ...
        _form.addLabel({
          id: 'eventDate',
          label: cachedProgramStage.executionDateLabel || 'Incident date',
        });
    ...
    if (cachedProgramStage.formType === 'CUSTOM' && cachedProgramStage.dataEntryForm) {
      ...
      try {
        ...
        log.error(
          errorCreator(ProgramStageFactory.CUSTOM_FORM_TEMPLATE_ERROR)(
            // $FlowFixMe
      ...
      const cachedProgramStageDataElementsAsObject = ProgramStageFactory._convertProgramStageDataElementsToObject(
        cachedProgramStage.programStageDataElements,
      );

      // $FlowFixMe[prop-missing] automated comment
      // $FlowFixMe[incompatible-use] automated comment
      await cachedProgramStage.programStageSections.asyncForEach(
        async (section: CachedProgramStageSection) => {
          stageForm.addSection(
            await this._buildSection(cachedProgramStageDataElementsAsObject, {
              id: section.id,
      ...
      stageForm.addSection(
        await this._buildMainSection(cachedProgramStage.programStageDataElements),
      );
    }
