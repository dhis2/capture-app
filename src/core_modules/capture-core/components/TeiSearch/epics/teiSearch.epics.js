...
import { actionTypes as programSelectorActionTypes } from '../SearchProgramSelector/searchProgramSelector.actions';
...
const getPagingQueryArgs = (pageNumber: ?number) =>
  pageNumber ? { page: pageNumber, pageSize: 5 } : { pageSize: 5 };
...
const searchTei = (
  ...
  const filterValues = searchGroup.searchForm.convertValues(formValues, (value, type, element) =>
  ...
  return getTrackedEntityInstances(queryArgs, attributes)
    .then((data) => searchTeiResultRetrieved(data, formId, searchGroupId, searchId))
  ...
  action$.pipe(
    ...
    switchMap((action) => {
      ...
      return from(searchTeiPromise).pipe(
        takeUntil(
          action$.pipe(
            ofType(actionTypes.REQUEST_SEARCH_TEI, actionTypes.TEI_SEARCH_RESULTS_CHANGE_PAGE),
          ),
        ),
        takeUntil(
          action$.pipe(
            filter(
              (ab) =>
                isArray(ab.payload) &&
                ab.payload.some((a) => a.type === actionTypes.INITIALIZE_TEI_SEARCH),
            ),
          ),
        ),
      );
...
export const teiSearchSetProgramEpic = (action$: InputObservable, store: ReduxStore) =>
  action$.pipe(
    ...
    map((action) => {
      ...
      const addFormDataActions = searchGroups
        ? searchGroups.map((sg, i) => {
...
export const teiNewSearchEpic = (action$: InputObservable, store: ReduxStore) =>
  action$.pipe(
    ...
    map((action) => {
      ...
      const contextId =
        currentTeiSearch.selectedProgramId || currentTeiSearch.selectedTrackedEntityTypeId;
