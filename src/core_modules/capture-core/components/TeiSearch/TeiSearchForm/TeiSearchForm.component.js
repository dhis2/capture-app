...
class SearchForm extends React.Component<Props> {
  ...
  validateForm() {
    if (!this.formInstance) {
      log.error(errorCreator(SearchForm.errorMessages.SEARCH_FORM_MISSING)({ Search: this }));
      return {
        error: true,
        isValid: false,
      };
  ...
  renderMinAttributesRequired = () => {
    ...
    const minAttributesRequiredClass = classNames(classes.minAttributesRequired, {
      [classes.minAttribtuesRequiredInvalid]: displayInvalidNumberOfAttributes,
    });

    return (
      <div className={minAttributesRequiredClass}>
        {i18n.t('Fill in at least {{minAttributesRequired}} attributes to search', {
  ...
  render() {
    ...
    if (!searchForm) {
      return <div>{SearchForm.errorMessages.SEARCH_FORM_MISSING}</div>;
    }
    ...
    return (
      <div className={classes.container}>
        ...
        <div className={classes.searchButtonContainer}>
          <Button onClick={this.handleSearchAttempt}>{searchButtonText}</Button>
