...
class StickyOnScroll extends React.Component<Props> {
  ...
  isNearTop = () =>
    window.pageYOffset + this.props.offsetTop > this.stickyContainer.parentElement.offsetTop;
  ...
  render() {
    ...
    return (
      <div className={containerClass || classes.container}>
        <div
          ref={(stickyContainerInstance) => {
            this.setStickyContainerInstance(stickyContainerInstance);
          }}
        >
          {children}
        </div>
