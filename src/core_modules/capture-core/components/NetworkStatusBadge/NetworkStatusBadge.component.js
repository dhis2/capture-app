...
class LeftSection extends PureComponent {
  ...
  render() {
    ...
    if (!props.status) {
      content = (
        <span>
          {props.syncList.length} {i18n.t('offline events. Last synced:')} {this.state.offlineTimer}
        </span>
      );
    }
    ...
    return (
      <Grow in={show} style={{ transformOrigin: 'right 50%' }}>
        <div
          className={`${props.classes.badgeSection} ${props.classes.smallText} ${props.classes.badgeSeparator}`}
          style={props.style}
        >
          {content}
        </div>
    ...
    return (
      <div className={`${classes.flex} ${classes.badgeContainer}`}>
        <LeftSection
          ...
          syncList={itemsToSync}
          classes={classes}
          statusTimer={this.props.networkStatus.offlineSince}
        />
