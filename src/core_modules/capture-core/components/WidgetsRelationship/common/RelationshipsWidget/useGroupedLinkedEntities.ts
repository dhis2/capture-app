...
const getFallbackFieldsByRelationshipEntity = (programStageLabel: string) => ({
    ...
    [RELATIONSHIP_ENTITIES.PROGRAM_STAGE_INSTANCE]: () => [{
        id: 'programStageName',
        displayName: tCustomTerm('{{programStageLabel}} name', { programStageLabel }),
...
const getColumns = ({ relationshipEntity, trackerDataView }: any, programStageLabel: string) => {
    let fields;
