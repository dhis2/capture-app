...
class ProgramSelector extends Component<Props> {
  ...
  getOptions() {
    ...
    if (this.props.selectedOrgUnitId) {
      programOptions = this.getOptionsFromPrograms(
        this.programsArray.filter(
          (program) =>
            program.organisationUnits[this.props.selectedOrgUnitId] && program.access.data.read,
        ),
      );
    } else {
      programOptions = this.getOptionsFromPrograms(
        this.programsArray.filter((program) => program.access.data.read),
      );
    }
  ...
  areAllProgramsAvailable(programOptions) {
    return (
      ...
      this.programsArray.filter((program) => program.access.data.read).length
    );
  }
    ...
    return (
      <>
        ...
        <div className={this.props.classes.selectedText}>
          <div className={this.props.classes.selectedTextAndIconContainer}>
        ...
        <div>
          <Paper elevation={0} className={classes.selectedPaper}>
            <Grid container spacing={8}>
              ...
              {
                // $FlowFixMe
                Array.from(selectedProgram.categoryCombination.categories.values()).map(
                  (category) => (
                          ...
                          return (
                            <div className={classes.selectedText}>
                              ...
                              <IconButton
                                ...
                                onClick={() => this.handleResetCategoryOption(category.id)}
                              >
  ...
  renderWithoutSelectedProgram(programOptions) {
    ...
    return (
      <Paper
        ...
        data-test="dhis2-capture-program-selector-container"
      >
        <h4 className={classes.title}>{i18n.t('Program')}</h4>
        {(() => {
          if (programOptions.length <= this.props.buttonModeMaxLength) {
            return <ProgramList items={programOptions} onSelect={handleClickProgram} />;
          }
          return (
            <div>
    ...
    return (
      <div>
        <Paper elevation={0} className={this.props.classes.paper}>
          ...
          <div className={this.props.classes.noProgramsContainer}>
  ...
  render() {
    ...
    const selectedProgram = this.props.selectedProgram
      ? programCollection.get(this.props.selectedProgram)
      : null;
    if (selectedProgram) {
