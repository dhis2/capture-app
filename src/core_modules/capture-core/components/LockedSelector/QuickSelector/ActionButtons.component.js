...
class ActionButtons extends Component<Props> {
  ...
  getButtonText = () => {
    if (this.props.selectedProgram) {
      const typeName =
        this.props.selectedProgram instanceof TrackerProgram
      ...
      <div className={classes.container}>
        {showResetButton ? (
          <div className={classes.startAgainContainer}>
            <Button onClick={this.handleStartAgainClick} small secondary>
              {i18n.t('Start again')}
            </Button>
          </div>
        ) : null}
        <Button onClick={this.handleNewClick}>
          <AddIcon data-test="dhis2-capture-new-event-button" className={classes.icon} />
          {this.getButtonText()}
        </Button>
