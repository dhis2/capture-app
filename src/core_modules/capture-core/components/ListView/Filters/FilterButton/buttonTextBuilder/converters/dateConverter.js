...
const convertToViewValue = (filterValue: string) =>
  pipe(
    (value) => moment(value),
...
function translateAbsoluteDate(filter: AbsoluteDateFilterData) {
  ...
  if (fromValue && toValue) {
    ...
    if (momentFrom.isSame(momentTo)) {
      ...
      appliedText = i18n.t('{{fromDate}} to {{toDate}}', {
        fromDate: appliedTextFrom,
        toDate: appliedTextTo,
      });
    }
  } else if (fromValue) {
    const appliedTextFrom = convertToViewValue(fromValue);
    appliedText = i18n.t('after or equal to {{date}}', {
      date: appliedTextFrom,
    });
  } else {
    // $FlowFixMe[incompatible-call] automated comment
    const appliedTextTo = convertToViewValue(toValue);
    appliedText = i18n.t('before or equal to {{date}}', {
      date: appliedTextTo,
    });
  }
  return appliedText;
