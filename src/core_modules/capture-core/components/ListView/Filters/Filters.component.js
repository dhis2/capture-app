...
const splitBasedOnHasValueOnInit = (
  ...
  return Object.keys(filtersNotEmpty).reduce(
    (acc, key) => {
...
const fillUpIndividualElements = (elementConfigs: Map<string, Column>, occupiedSpots: number) => {
...
const getUserSelectedElements = (
  ...
  return Object.keys(userSelectedFiltersNonEmpty).reduce(
    (acc, key) => {
      const config = elementConfigs.get(key);
      if (!config) {
        log.error(
...
const getIndividualElementsArray = (
  ...
  [...validElementConfigs.entries()]
    .map((entry) => entry[1])
    .map((element) => {
      if (
        initValueElements.has(element.id) ||
...
const renderIndividualFilterButtons = ({
  ...
  individualElementsArray.map(({ id, type, header, options, multiValueFilter }) => (
    <div
      key={id}
...
const renderRestButton = (restElementsArray: Array<Column>, onSelectRestMenuItem: Function) =>
...
const FiltersPlain = memo<Props>((props: Props) => {
  ...
  const elementsContainer = React.useMemo(() => {
    ...
    const {
      initValueElements,
      remainingElements: remainingElementsAfterInitSplit,
    } = splitBasedOnHasValueOnInit(validElementConfigs, stickyFilters.filtersWithValueOnInit);

    const {
      fillUpElements,
      remainingElements: remainingElementsAfterFillUp,
    } = fillUpIndividualElements(remainingElementsAfterInitSplit, initValueElements.size);

    const { userSelectedElements, remainingElements } = getUserSelectedElements(
      remainingElementsAfterFillUp,
      stickyFilters.userSelectedFilters,
    );

    const individualElementsArray = getIndividualElementsArray(
      validElementConfigs,
      initValueElements,
      fillUpElements,
  ...
  }, [columns, stickyFilters]);

  const handleRestMenuItemSelected = React.useCallback(
    (id: string) => {
      setVisibleSelector(id);
      onSelectRestMenuItem(id);
    },
    [setVisibleSelector, onSelectRestMenuItem],
  );
  ...
  const filterButtons = React.useMemo(() => {
    ...
    const restButton = renderRestButton(restElementsArray, handleRestMenuItemSelected);

    return [[...individualFilterButtons], restButton];
  }, [
    elementsContainer,
    visibleSelectorId,
    setVisibleSelector,
