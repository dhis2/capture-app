...
const buildFormOptionSet = (clientOptionSet: OptionSet) => {
  if (!clientOptionSet.dataElement) {
    log.error(
      errorCreator(errorMessages.DATAELEMENT_MISSING)({
        clientOptionSet,
      }),
    );
    return null;
...
const flattenOptionSetForRadioButtons = (formOptionSet: OptionSet) =>
...
const flattenOptionSetForSelect = (formOptionSet: OptionSet) =>
  formOptionSet.options.map((option) => ({
    id: option.id,
    label: option.text,
    value: option.value,
