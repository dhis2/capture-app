...
const cancelActionFilter = (action: Object, formId: string, elementId: string) => {
  ...
  return (
    isAddFormData(action, formId) || isRequestFilterFormFieldOrgUnits(action, formId, elementId)
  );
};
      ...
      return from(
        getD2()
          ...
          .then((orgUnitCollection) => ({
            orgUnitArray: orgUnitCollection.toArray(),
            searchText,
            formId,
            elementId,
          }))
          .catch((error) => ({ error, formId, elementId })),
      ).pipe(takeUntil(action$.pipe(filter((a) => cancelActionFilter(a, formId, elementId)))));
    }),
    map((resultContainer) => {
      if (resultContainer.error) {
        log.error(
          errorCreator(FILTER_RETRIEVE_ERROR)({
            error: resultContainer.error,
            method: 'FilterOrgUnitRootsEpic',
          }),
        );
      ...
      const orgUnits = orgUnitArray.map((unit) => ({
        id: unit.id,
        path: unit.path,
