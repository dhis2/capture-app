...
const getSearchGroupsHOC = (
  ...
  class SearchGroupsHOC extends React.Component<Props> {
    ...
    getFilterActions() {
      ...
      const searchUids = searchGroups.map(() => uuid());

      // $FlowFixMe[missing-annot] automated comment
      const filterActions = searchGroups.map((sg, index) =>
        filterSearchGroupForCountSearch(sg, searchUids[index], dataEntryKey, searchContext),
      );

      // $FlowFixMe[missing-annot] automated comment
      const filterActionsToBeExecuted = filterActions.map((fa) =>
        filterSearchGroupForCountSearchToBeExecuted(fa),
      );
    ...
    handleFieldUpdate = (innerAction: ReduxAction<any, any>) => {
      const { onUpdateFormField, onUpdateFormFieldInner } = this.props;

      onUpdateFormFieldInner(innerAction, this.getFilterActions(), onUpdateFormField);
    };

    // $FlowFixMe[missing-annot] automated comment
    handleAsyncFieldUpdate = (...args) => {
      const { onUpdateFormFieldAsync, onUpdateFormFieldInnerAsync } = this.props;
      onUpdateFormFieldInnerAsync(...args, this.getFilterActions(), onUpdateFormFieldAsync);
    };
...
const mapDispatchToProps = (dispatch: ReduxDispatch) => ({
  onUpdateFormFieldInner: (
    ...
    if (onUpdateFormField) {
      onUpdateFormField(innerAction, { filterActions, filterActionsToBeExecuted });
    } else {
      dispatch(
        updateFieldAndRunSearchGroupSearchesBatch(
          innerAction,
