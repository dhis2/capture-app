  ...
  class WarningOutputBuilder extends React.Component<Props> {
    ...
    getVisibleWarningItems() {
      ...
      if (saveAttempted) {
        ...
        return [...warningItemsNoNull, ...warningOnCompleteItemsNoNull];
      }
          ...
          {visibleItems && visibleItems.length > 0 && (
            <Card className={classes.card}>
              <div className={classes.header}>
                <Warning />
                <div className={classes.headerText}>{i18n.t('Warnings')}</div>
              </div>
...
const mapStateToProps = (state: ReduxState, props: any) => {
  ...
  return {
    warningItems: state.rulesEffectsGeneralWarnings[key]
      ? state.rulesEffectsGeneralWarnings[key].warning
      : null,
    warningOnCompleteItems: state.rulesEffectsGeneralWarnings[key]
      ? state.rulesEffectsGeneralWarnings[key].warningOnComplete
      : null,
  };
};
