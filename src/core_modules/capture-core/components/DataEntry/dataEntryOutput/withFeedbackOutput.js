...
const getFeedbackOutput = () =>
  class FeedbackOutputBuilder extends React.Component<Props> {
      ...
      <div>
        {feedbackItems.displayTexts &&
          feedbackItems.displayTexts.map((item) => (
            <ListItem key={item.id} className={classes.listItem} button={false}>
    ...
    render = () => {
      ...
      const hasItems =
        feedbackItems && (feedbackItems.displayTexts || feedbackItems.displayKeyValuePairs);
      return (
        <div>
          {hasItems && (
            <Card className={classes.card}>
              {i18n.t('Feedback')}
              <List dense>{feedbackItems && this.renderFeedbackItems(feedbackItems, classes)}</List>
            </Card>
          )}
        </div>
      );
...
const mapStateToProps = (state: ReduxState, props: any) => {
  ...
  return {
    feedbackItems:
      state.rulesEffectsFeedback && state.rulesEffectsFeedback[key]
        ? state.rulesEffectsFeedback[key]
        : null,
  };
};
