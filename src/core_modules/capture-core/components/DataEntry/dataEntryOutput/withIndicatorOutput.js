...
const getIndicatorOutput = () =>
  class IndicatorkOutputBuilder extends React.Component<Props> {
    ...
    render = () => {
      ...
      const hasItems =
        indicatorItems && (indicatorItems.displayTexts || indicatorItems.displayKeyValuePairs);
      return (
        <div>
          {hasItems && (
...
const mapStateToProps = (state: ReduxState, props: any) => {
  ...
  return {
    indicatorItems:
      state.rulesEffectsIndicators && state.rulesEffectsIndicators[key]
        ? state.rulesEffectsIndicators[key]
        : null,
  };
};
