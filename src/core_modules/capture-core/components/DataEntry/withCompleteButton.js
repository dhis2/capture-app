...
const getCompleteButton = (InnerComponent: React.ComponentType<any>, optionFn?: ?OptionFn) =>
  class CompleteButtonBuilder extends React.Component<Props, State> {
    ...
    getFormInstance() {
      ...
      while (!done) {
        currentInstance =
          currentInstance.getWrappedInstance && currentInstance.getWrappedInstance();
    ...
    validateForm() {
      ...
      if (!formInstance) {
        log.error(
          errorCreator(CompleteButtonBuilder.errorMessages.FORM_INSTANCE_NOT_FOUND)({
            CompleteButtonBuilder: this,
          }),
        );
        return;
      }
    ...
    handleCompletionAttempt() {
      if (!this.innerInstance) {
        log.error(
          errorCreator(CompleteButtonBuilder.errorMessages.INNER_INSTANCE_NOT_FOUND)({
            CompleteButtonBuilder: this,
          }),
        );
        return;
    ...
    render() {
      ...
      return (
        <div>
          ...
          <Dialog open={this.state.warningDialogOpen} onClose={this.handleCloseDialog}>
            <DialogTitle id="complete-dialog-title">{i18n.t('Warnings found')}</DialogTitle>
            <DialogContent>
              <DialogContentText>{this.getDialogWarningContents()}</DialogContentText>
            </DialogContent>
            <DialogActions>
...
const mapStateToProps = (state: ReduxState, props: { id: string }) => {
  const eventId =
    state.dataEntries && state.dataEntries[props.id] && state.dataEntries[props.id].eventId;
  ...
  return {
    ...
    warnings:
      state.rulesEffectsMessages[eventId] &&
      Object.keys(state.rulesEffectsMessages[eventId])
        .map((elementId) => {
          const warning =
            state.rulesEffectsMessages[eventId][elementId] &&
...
export default (optionFn?: ?OptionFn) => (InnerComponent: React.ComponentType<any>) =>
  // $FlowFixMe[missing-annot] automated comment
  connect(mapStateToProps, mapDispatchToProps)(getCompleteButton(InnerComponent, optionFn));
