...
function getServerValues(updatedFormValues: Object, foundation: RenderFoundation) {
  const convertFn = pipeD2(convertFormToClient, convertClientToServer);
...
function executeSearch(
  ...
  const searchGroupValues = searchGroup.searchFoundation.getElements().reduce((acc, element) => {
    acc[element.id] = updatedFormValues[element.id];
    return acc;
  }, {});

  return searchGroup.onSearch(
...
function searchHasValidAndRequiredValues(
  searchGroup: InputSearchGroup,
  values: Object,
  fieldsUI: Object,
) {
  ...
  if (
    elements.filter(
      (e) => fieldsUI[e.id] && fieldsUI[e.id].errorType === 'dataType' && !fieldsUI[e.id].valid,
    ).length > 0
  ) {
    return false;
  }

  const elementsWithValue = elements.filter((e) => values[e.id]);
  return elementsWithValue.length >= searchGroup.minAttributesRequiredToSearch;
}

function searchHasUpdatedValues(
  searchGroup: InputSearchGroup,
  values: Object,
...
export const getFilterSearchGroupForSearchEpic = (triggerBatches: Array<string>) => (
  ...
  action$.pipe(
    ...
    map((actionBatch) => {
      const potentialSearchActions = actionBatch.payload.filter(
      ...
      const outputActions = potentialSearchActions
        .map((sa) => {
          ...
          const previousValues =
            (state.dataEntriesSearchGroupsPreviousValues[dataEntryKey] &&
              state.dataEntriesSearchGroupsPreviousValues[dataEntryKey][searchGroup.id]) ||
            {};
          const fieldsUI = Object.keys(state.formsSectionsFieldsUI)
          ...
          const abortSearchGroupSearch = !searchHasValidAndRequiredValues(
            searchGroup,
            formValues,
            fieldsUI,
          );
        ...
        .map((sa) => {
          if (sa.abort) {
            ...
            const currentlyActiveUids =
              (saveWaitUids[dataEntryKey] && saveWaitUids[dataEntryKey][searchGroup.id]) || [];
            cleanUpUidsAfterSearch(dataEntryKey, searchGroup.id);
            return abortSearchGroupCountSearch(dataEntryKey, searchGroup, searchGroup.id, [
              ...currentlyActiveUids,
              uid,
            ]);
          }
...
export const getExecuteSearchForSearchGroupEpic = (cancelBatches: Array<string>) => (
  action$: ActionsObservable,
  store: ReduxStore,
) =>
  action$.pipe(
    ...
    map((actionBatch) => {
      ...
      return actions.filter(
        (action) => action.type === searchActionTypes.START_SEARCH_GROUP_COUNT_SEARCH,
      );
    }),
    filter((actions) => actions && actions.length > 0),
    mergeMap((searchActions) =>
      searchActions.map((searchAction) => {
        ...
        return race(
          from(executeSearch(searchGroup, formValues, contextProps)).pipe(
            takeUntil(
              action$.pipe(
                ofType(searchBatchActionTypes.FILTERED_SEARCH_ACTIONS_FOR_SEARCH_BATCH),
                filter((ab) =>
                  ab.payload.find(
                    (a) =>
                      ...
                      ].includes(a.type) && a.payload.searchGroup === searchGroup,
                  ),
                ),
              ),
            ),
          ...
          action$.pipe(
            ...
            filter((ab) =>
              ab.payload.find(
                (a) =>
                  a.type ===
                    (loadNewActionTypes.LOAD_NEW_DATA_ENTRY ||
                      loadEditActionTypes.LOAD_EDIT_DATA_ENTRY) && a.payload.key === dataEntryKey,
              ),
            ),
            map(() => {
