...
export default (state: ReduxState, key: string) => {
  const reduced = Object.keys(state.formsSectionsFieldsUI)
    ...
    .reduce(
      (accElementsUI, sectionKey) => [
        ...accElementsUI,
        ...Object.keys(state.formsSectionsFieldsUI[sectionKey]).map(
          (elementKey) => state.formsSectionsFieldsUI[sectionKey][elementKey],
        ),
      ],
      [],
    );
  ...
  const dataEntryIsModified = UIDataForDataValues
    ? Object.keys(UIDataForDataValues).some(
        (elementKey) => UIDataForDataValues[elementKey].modified,
      )
    : false;
