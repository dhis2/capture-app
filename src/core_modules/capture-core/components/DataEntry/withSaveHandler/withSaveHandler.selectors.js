...
const boundMessagesSelector = (state, props, calculatedValues) =>
...
export const makeGetWarnings = () =>
  ...
  createSelector(
    ...
    (boundMessages, generalWarningsOnComplete, foundation) => {
      ...
      const boundWarnings = Object.keys(boundMessages)
        .map((elementId) => {
      ...
      const unboundWarnings = generalWarningsOnComplete.map((w) => ({
        key: w.id,
        name: null,
        warning: w.message,
      }));
...
export const makeGetErrors = () =>
  ...
  createSelector(
    ...
    (boundMessages, generalErrorsOnComplete, foundation) => {
      ...
      const boundErrors = Object.keys(boundMessages)
        .map((elementId) => {
          const messages = boundMessages[elementId];
      ...
      const unboundErrors = generalErrorsOnComplete.map((e) => ({
        key: e.id,
        name: null,
        error: e.message,
      }));
