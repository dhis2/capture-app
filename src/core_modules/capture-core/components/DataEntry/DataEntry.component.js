...
type Props = {
  ...
  onUpdateFieldInner: (action: ReduxAction<any, any>) => void,
  ...
  dataEntrySections?: {
    [string]: { name: string, placement: $Values<typeof placements> },
  },
...
class DataEntry extends React.Component<Props> {
      ...
      ? Object.keys(this.props.dataEntrySections).reduce((accSections, sectionKey) => {
          ...
          if (section.placement === placement) {
            ...
            if (sectionFields && sectionFields.length > 0) {
              accSections.push(
                <div key={sectionKey} className={this.props.classes.dataEntryFieldSectionContainer}>
                  <Section header={<SectionHeaderSimple title={section.name} />}>
                    {sectionFields}
                  </Section>
                </div>,
              );
  ...
  renderDataEntryFieldsByPlacement = (placement: $Values<typeof placements>) => {
    const fields = this.props.fields || [];
    const fieldFilter = this.props.formHorizontal
      ? fieldHorizontalFilter(placement)
      : fieldVerticalFilter(placement);
    const fieldsByPlacement = fields
      ? fields.filter(fieldFilter).map((fieldContainer) => fieldContainer.field)
      : [];
  ...
  renderD2Form = () => {
    ...
    return this.props.formHorizontal ? (
      d2Form
    ) : (
      <div className={classes.d2FormContainer}>{d2Form}</div>
    );
  };
  ...
  render() {
    ...
    if (!itemId) {
      return <div>{DataEntry.errorMessages.NO_ITEM_SELECTED}</div>;
    }

    if (!formFoundation) {
      return <div>{DataEntry.errorMessages.FORM_FOUNDATION_MISSING}</div>;
    }
      ...
      <div className={directionClasses.container}>
        <div className={directionClasses.dataEntryContainer}>
          ...
          {!this.props.formHorizontal && dataEntryOutputs ? (
            ...
            >
              <div>{dataEntryOutputs}</div>
            </StickyOnScroll>
          ) : (
            <div className={classes.verticalOutputsContainer}>{dataEntryOutputs}</div>
          )}
        </div>
        <div className={classes.footerBar}>
          {(() => {
            if (completeButton) {
              return <div className={classes.button}>{completeButton}</div>;
            }
            return null;
          })()}
          ...
          {(() => {
            if (mainButton) {
              return <div className={classes.button}>{mainButton}</div>;
            }
            return null;
          })()}
          ...
          {(() => {
            if (cancelButton) {
              return <div className={classes.button}>{cancelButton}</div>;
            }
            return null;
          })()}
        </div>
