...
export const openDataEntryForNewEnrollmentBatchAsync = async (
  ...
  const dataEntryActions = loadNewDataEntry(
    ...
    null,
    generatedItemContainers.reduce((accValuesByKey, container) => {
  ...
  if (program && foundation) {
    rulesActions = getRulesActionsForTEI(program, foundation, formId, orgUnit, {}, {});
  } else {
    rulesActions = [];
  }
  ...
  return batchActions(
    [
      openDataEntryForNewEnrollment(
        dataEntryId,
        generatedItemContainers.reduce((accItemsByKey, container) => {
