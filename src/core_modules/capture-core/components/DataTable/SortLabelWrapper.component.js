...
class SortLabelWrapper extends React.Component<Props> {
  ...
  getActiveIcons = (
    ...
    const IconComponent =
      direction === sortLabelDirections.ASC ? ArrowUpwardIcon : ArrowDownwardIcon;
    const icon = (
      <IconComponent
        className={
          isDisabled ? classes.iconBase : classNames(classes.iconBase, classes.enabledIcon)
        }
        onClick={this.getIconClickHandler(
          direction === sortLabelDirections.DESC
    ...
    return (
      <Tooltip title={i18n.t('Sort')} placement="bottom" enterDelay={500}>
        <span>{icon}</span>
      </Tooltip>
    );
  };
  ...
  render() {
    return (
      ...
      <SortLabel onGetIcons={this.getIcons} {...this.props}>
        {(() => {
          if (this.props.disabled) {
