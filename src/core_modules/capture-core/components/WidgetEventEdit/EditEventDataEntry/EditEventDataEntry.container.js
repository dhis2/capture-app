...
const mapDispatchToProps = (dispatch: ReduxDispatch, props): any => ({
    ...
    onCancelCreateNew: (itemId: string) => {
        ...
        dispatch(startCreateNewAfterCreating({ enrollmentId, orgUnitId: orgUnit.id, programId, teiId }));
    },
    onConfirmCreateNew: (itemId: string) => {
        const { dataEntryId, formFoundation, orgUnit, enrollmentId, programId, teiId } = props;
        dispatch(requestSaveEditEventDataEntry(itemId, dataEntryId, formFoundation, orgUnit));
        dispatch(startCreateNewAfterCreating({ enrollmentId, isCreateNew: true, orgUnitId: orgUnit.id, programId, teiId }));
    },
});
