...
const deriveEnrollmentType = (enrollments, currentProgramId): $Keys<typeof enrollmentTypes> => {
  ...
  if (enrollmentsInCurrentProgram.find(({ status }) => status === COMPLETED)) {
    return COMPLETED;
  }
  if (enrollmentsInCurrentProgram.find(({ status }) => status === CANCELLED)) {
    return CANCELLED;
  }
  return enrollmentTypes.DONT_SHOW_TAG;
};
...
const deriveEnrollmentOrgUnitAndDate = (
  ...
  enrollmentType,
  currentProgramId,
): { orgUnitName?: string, enrollmentDate?: string } => {
  if (!currentProgramId) {
    return {};
  }
...
const CardListItemIndex = ({
  ...
  const renderImageDataElement = (imageElement: CardProfileImageElementInformation) => {
    ...
    return (
      <div>
        {imageValue && (
          <Avatar src={imageValue.url} alt={imageValue.name} className={classes.image} />
        )}
      </div>
    );
  };
  ...
  const { orgUnitName, enrollmentDate } = deriveEnrollmentOrgUnitAndDate(
    enrollments,
    enrollmentType,
    currentProgramId,
  );
    ...
    <div data-test="dhis2-capture-card-list-item" className={classes.itemContainer}>
      <div className={classes.itemDataContainer}>
        <div className={classes.itemValuesContainer}>
          <Grid container>
            {profileImageDataElement && (
              <Grid item>{renderImageDataElement(profileImageDataElement)}</Grid>
            )}
            <Grid item sm container>
              <Grid item xs container direction="column">
                {dataElements.map(({ id, name, type }) => (
                  <ListEntry key={id} name={name} value={item.values[id]} type={type} />
                ))}
                {orgUnitName && (
