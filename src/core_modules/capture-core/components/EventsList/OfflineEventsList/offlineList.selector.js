...
export const makeColumnsSelector = () =>
  ...
  createSelector(
    ...
    (programId, columnsOrderFromState: ColumnsOrderFromState) => {
      ...
      return columnsOrderFromState.map((column) => {
...
const createEventsContainer = (events, eventsValues, sortOrder): Array<EventContainer> =>
  sortOrder.map((eventId) => ({
    event: events[eventId],
    eventValues: eventsValues[eventId],
  }));

export const makeCreateEventsContainer = () =>
  // $FlowFixMe[missing-annot] automated comment
  createSelector(
...
const onIsEventsEqual = (
  ...
  currentEventContainers.every(
    (eventContainer, index) =>
      ...
      eventContainer.eventValues ===
        (prevEventContainers[index] && prevEventContainers[index].eventValues),
  ) &&
  prevEventContainers.every(
    (eventContainer, index) =>
      eventContainer.event ===
        (currentEventContainers[index] && currentEventContainers[index].event) &&
      eventContainer.eventValues ===
        (currentEventContainers[index] && currentEventContainers[index].eventValues),
  );

const createDeepEqualSelector = createSelectorCreator(defaultMemoize, onIsEventsEqual);

const buildWorkingListData = (eventsContainer: Array<EventContainer>, columns) => {
  ...
  return eventsContainer.map((eventContainer) => {
      ...
      .reduce((acc, mainColumn) => {
        ...
        if (sourceValue) {
          if (mainColumn.options) {
            ...
            if (!option) {
              log.error(
                errorCreator('Missing value in options for main values')({
                  sourceValue,
                  mainColumn,
                }),
              );
...
export const makeCreateWorkingListData = () =>
  // $FlowFixMe[missing-annot] automated comment
  createDeepEqualSelector(
    (eventContainers, columns) => ({ eventContainers, columns }),
    ({ eventContainers, columns }) => buildWorkingListData(eventContainers, columns),
  );
