...
class DateFilter extends Component<Props, State> implements UpdatableFilterContent<Value> {
  ...
  static isFilterValid(mainValue?: ?string, fromValue?: ?string, toValue?: ?string) {
  ...
  getErrors() {
    ...
    if (mainValue === mainOptionKeys.CUSTOM_RANGE && !fromValue && !toValue) {
      return {
        minValueError: null,
        maxValueError: null,
        logicError: submitAttempted
          ? i18n.t(DateFilter.errorMessages.CUSTOM_RANGE_WITHOUT_VALUES)
          : null,
      };
    }

    const { isValid: isMinValueValid, error: minValueError } = DateFilter.validateField(
      fromValue,
      type,
    );
    const { isValid: isMaxValueValid, error: maxValueError } = DateFilter.validateField(
      toValue,
      type,
    );
