...
class OptionsSelectVirtualized extends React.Component<Props, State> {
  static getFilteredOptions(
    options: Array<VirtualizedOptionConfig>,
    filterValue: any,
  ): Array<VirtualizedOptionConfig> {
    const filterValueLC = filterValue.toLowerCase();
    return (
      (options && options.filter((o) => o.label.toLowerCase().indexOf(filterValueLC) > -1)) || []
    );
  }
  ...
  filterOptions = () => {
    ...
    if (
      ...
      this.prevFilteredOptions = OptionsSelectVirtualized.getFilteredOptions(
        this.prevFilteredOptions,
        this.state.filterValue,
      );
    }
  ...
  renderValue = (option: VirtualizedOptionConfig) => {
    ...
    return (
      <div className={classes.selectedOptionContainer}>
        {iconLeft ? <div className={classes.selectedIconLeftContainer}>{iconLeft}</div> : null}
        <div>{label}</div>
        {iconRight ? <div className={classes.selectedIconRightContainer}>{iconRight}</div> : null}
      </div>
    );
  };
  ...
  render() {
    ...
    const selectStyle = {
      ...OptionsSelectVirtualized.defaultSelectStyle,
      ...style,
    };
    const menuContainerStyle = {
      ...OptionsSelectVirtualized.defaultMenuContainerStyle,
      ...menuStyle,
    };
    return (
      <div
