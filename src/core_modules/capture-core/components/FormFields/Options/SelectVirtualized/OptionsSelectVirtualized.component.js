...
class OptionsSelectVirtualized extends Component<Props, State> {
  ...
  buildOptions(optionSet: OptionSet) {
    const options = optionSet.options.map((o: Option) => ({
      label: o.text,
      value: o.value,
    }));
    return options;
  ...
  render() {
    ...
    const lineClasses = classNames(classes.underline, classes.inkBar, {
      [classes.focused]: inFocus,
    });
