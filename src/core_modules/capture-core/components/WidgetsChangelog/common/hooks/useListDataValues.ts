...
const fetchFormattedValues = async ({
    ...
    fromServerDate: (date: string) => Date;
}) => {
    ...
    const results = await Promise.all(
        rawRecords.changeLogs.map(async (changelog) => {
            ...
            const getValue = async (value: any, isLatestValue: boolean) => {
                if (!getSubValue) {
            ...
            const previousValue = convertClientToList(previousValueClient, metadataElement.type, options || null);
            const currentValue = convertClientToList(currentValueClient, metadataElement.type, options || null);
