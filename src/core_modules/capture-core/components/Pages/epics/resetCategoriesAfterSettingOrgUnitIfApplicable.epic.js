...
async function isOptionAssociatedWithOrganisationUnit(categoryOptionId: string, orgUnitId: string) {
  const categoryOption = await getUserStorageController().get(
    userStores.CATEGORY_OPTIONS,
    categoryOptionId,
  );
...
export const resetCategoriesAfterSettingOrgUnitIfApplicableEpic = (
  ...
  action$.pipe(
    ...
    switchMap((action) => {
      ...
      const categoriesWithValue = Object.keys(selectedCategories).reduce((acc, categoryId) => {
      ...
      const isAssociatedPromises = Object.keys(categoriesWithValue).map((categoryId) => {
        const categoryOptionId = categoriesWithValue[categoryId];
        return isOptionAssociatedWithOrganisationUnit(categoryOptionId, orgUnitId).then(
          (isAssociated) => ({
            id: categoryId,
            isAssociated,
          }),
      ...
      return Promise.all(isAssociatedPromises).then((associatedContainers) => {
