...
const buildSearchOption = (id, name, searchGroups, searchScope, type) => ({
  ...
  TETypeName: type,
  searchGroups: [...searchGroups.values()].map(
    ({ unique, searchForm, minAttributesRequiredToSearch }, index) => ({
      unique,
      searchForm,
      ...
      Object.values(trackedEntityTypesWithCorrelatedPrograms)
        .reduce(
          (
            ...
            { trackedEntityTypeId, trackedEntityTypeName, trackedEntityTypeSearchGroups, programs },
          ) => ({
            ...acc,
...
const usePreselectedProgram = (): ?string => {
  const currentSelectionsId = useSelector(({ currentSelections }) => currentSelections.programId);
  const trackedEntityTypesWithCorrelatedPrograms = useTrackedEntityTypesWithCorrelatedPrograms();
  ...
  return useMemo(() => {
    const { programId } =
      Object.values(trackedEntityTypesWithCorrelatedPrograms)
        ...
        .map(({ programs }) =>
          programs.find(
            ({ programId: currentProgramId }) => currentProgramId === currentSelectionsId,
          ),
        )
...
export const SearchPage: ComponentType<{||}> = () => {
  ...
  const searchStatus: string = useSelector(({ searchPage }) => searchPage.searchStatus);
  const error: boolean = useSelector(
    ({ activePage }) => activePage.selectionsError && activePage.selectionsError.error,
  );
  const ready: boolean = useSelector(({ activePage }) => !activePage.isLoading);
  const currentProgramId: string = useSelector(
    ({ currentSelections }) => currentSelections.programId,
  );
