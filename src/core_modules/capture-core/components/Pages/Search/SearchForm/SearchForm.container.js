...
const isValueContainingCharacter = (value: any) => {
  ...
  if (isObject(value)) {
    const numberOfValuesWithLength = Object.values(value)
      .filter((v) => isString(v))
      .filter((v: any) => Boolean(v.replace(/\s/g, '').length)).length;
...
const collectCurrentSearchTerms = (
  ...
  const { searchForm: attributeSearchForm, formId } = searchGroupsForSelectedScope.reduce(
    (accumulated, searchGroup) => {
  ...
  return Object.keys(searchTerms).reduce((accumulated, attributeValueKey) => {
...
const mapStateToProps = (
  state: ReduxState,
  { searchGroupsForSelectedScope }: OwnProps,
): PropsFromRedux => {
  const {
    formsValues,
    searchPage: { searchStatus, keptFallbackSearchFormValues },
  } = state;
  ...
  return {
    ...
    isSearchViaAttributesValid: (minAttributesRequiredToSearch) => {
      const currentSearchTerms = collectCurrentSearchTerms(
        searchGroupsForSelectedScope,
        formsValues,
      );
...
const mapDispatchToProps = (
  ...
  removeFormDataFromReduxStore: () => {
    searchGroupsForSelectedScope.forEach(({ formId }) => {
      dispatch(removeFormData(formId));
    });
  },
});

export const SearchForm: ComponentType<OwnProps> = connect<
