  ...
  from(getTrackedEntityInstances(queryArgs, attributes)).pipe(
    map(({ trackedEntityInstanceContainers: searchResults, pagingData }) => {
      ...
      if (searchResults.length === 0 && triggeredFrom === PAGINATION) {
        return showSuccessResultsViewOnSearchPage([], pagingData.currentPage);
      }
...
export const searchViaUniqueIdOnScopeProgramEpic: Epic = (action$, store) =>
  action$.pipe(
    ...
    flatMap(({ payload: { formId, programId } }) => {
      ...
      const { attributes } = getTrackerProgramThrowIfNotFound(programId);
...
export const searchViaUniqueIdOnScopeTrackedEntityTypeEpic: Epic = (action$, store) =>
  action$.pipe(
    ...
    flatMap(({ payload: { formId, trackedEntityTypeId } }) => {
      ...
      const { attributes } = getTrackedEntityTypeThrowIfNotFound(trackedEntityTypeId);
...
const deriveSearchFormInfo = (searchGroups) =>
  searchGroups.filter((searchGroup) => !searchGroup.unique)[0] || {};
    ...
    flatMap(({ payload: { formId, programId, pageSize, availableSearchOptions } }) => {
      ...
      if (trackerProgram.trackedEntityType) {
        ...
        const {
          formsValues,
          searchPage: {
            currentSearchInfo: { currentSearchTerms: searchTermsFromOriginalSearch },
          },
        } = store.value;

        const { searchForm, formId: fallbackFormId } = deriveSearchFormInfo(
          availableSearchOptions[trackedEntityTypeId].searchGroups,
        );
        const fallbackFormValues = deriveFormValues(searchForm, formsValues[formId]);
        const fallbackSearchTerms = deriveCurrentFallbackSearchTerms(
          searchTermsFromOriginalSearch,
          fallbackFormValues,
        );
