...
const buttonDefinitions = {
  [buttonTypes.SAVEANDADDANOTHER]: (props: Props) => ({
    ...
    onClick: () => {
      props.onSave(newEnrollmentSaveTypes.SAVEANDADDANOTHER);
    },
  }),
  [buttonTypes.SAVEANDEXIT]: (props: Props) => ({
    ...
    onClick: () => {
      props.onSave(newEnrollmentSaveTypes.SAVEANDEXIT);
    },
  }),
...
const getMainButton = (InnerComponent: React.ComponentType<any>) =>
  class MainButtonHOC extends React.Component<Props> {
    ...
    getButtonDefinition = (type: $Values<typeof buttonTypes>) =>
      buttonDefinitions[type](this.props);

    getFormHorizontalButtons = (
      dataEntryHasChanges: ?boolean,
      hasRecentlyAddedEvents: ?boolean,
    ) => {
      const buttons = [
