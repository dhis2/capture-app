...
function getTrackerProgram(suggestedProgramId: string) {
    let trackerProgram: TrackerProgram | undefined;
    try {
        ...
        throw Error(i18n.t('Metadata error. see log for details') as string);
    }
    return trackerProgram;
}

type InputObservable = Observable<any>;
...
export const openRelationshipTeiRegisterWidgetEpic = (action$: InputObservable) =>
    action$.pipe(
        ...
        switchMap((action) => {
            ...
            if (!orgUnit) {
                return Promise.resolve(initializeRegisterTei(trackerProgram ? trackerProgram.id : null));
            }
