...
type Props = {
  ...
  feedbacks: ?{
    ...
    displayKeyValuePairs: ?Array<{ key: string, value: string }>,
  },
};
...
class FeedbacksSection extends React.Component<Props> {
  ...
  renderTextItems = (displayTexts: Array<any>, classes: Object) =>
    displayTexts.map((displayText) => (
      <div className={classNames(classes.feedback, classes.textFeedback)} key={displayText.id}>
        {displayText.message}
      </div>
    ));

  renderKeyValueItems = (keyValuePairs: Array<any>, classes: Object) =>
