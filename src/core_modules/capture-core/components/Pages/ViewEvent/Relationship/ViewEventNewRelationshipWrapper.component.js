...
class ViewEventNewRelationshipWrapper extends React.Component<Props, State> {
  ...
  renderHeader = () => (
    <div className={this.props.classes.headerContainer}>
      <div className={this.props.classes.header}>{i18n.t('New event relationship')}</div>
    </div>
  );
    ...
    return (
      <div className={classes.container}>
        <div className={classes.backToEventContainer}>
          ...
          <LinkButton className={classes.backToEventButton} onClick={this.handleDiscard}>
            {i18n.t('Go back to event without saving relationship')}
          </LinkButton>
        </div>
        <Paper className={classes.newRelationshipPaper}>
          {/* $FlowFixMe[cannot-spread-inexact] automated comment */}
          <NewRelatonship header={i18n.t('New event relationship')} {...passOnProps} />
        </Paper>
        <ConfirmDialog
          header={i18n.t('Unsaved changes')}
          text={i18n.t(
            'Leaving this page will discard any selections you made for a new relationship',
          )}
