...
export const NewPage: ComponentType<{||}> = () => {
  ...
  const currentScopeId: string = useSelector(
    ({ currentSelections }) => currentSelections.programId || currentSelections.trackedEntityTypeId,
  );

  const currentOrgUnitId: string = useSelector(
    ({ currentSelections }) => currentSelections.orgUnitId,
  );

  const newPageStatus: $Keys<newPageStatuses> = useSelector(({ newPage }) => newPage.newPageStatus);
