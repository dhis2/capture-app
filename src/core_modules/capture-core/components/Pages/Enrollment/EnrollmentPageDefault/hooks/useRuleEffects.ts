...
const getDataElementsInProgram = (program: TrackerProgram) => [...program.stages.values()]
    .reduce((acc, {
        stageForm,
    }) => ({ ...acc,
...
const getClientFormattedDataValuesAsObject = (dataValues, elementsById) => dataValues.reduce((acc, {
    dataElement: id,
    value,
}) => {
...
export const useRuleEffects = ({
    ...
    const attributesObject = useMemo(() => program.attributes.reduce((acc, attribute) => {
        acc[attribute.id] = attribute;
        return acc;
    }, {}), [program.attributes]);
    const attributeValues = useMemo(() => apiAttributeValues && apiAttributeValues.reduce((acc, {
        id,
        value,
    }) => {
