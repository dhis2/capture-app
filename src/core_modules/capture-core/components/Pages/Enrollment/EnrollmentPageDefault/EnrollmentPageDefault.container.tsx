...
import { useProgramMetadata, useHideWidgetByRuleLocations, useProgramStages, useRuleEffects } from './hooks';
...
export const EnrollmentPageDefault = () => {
    ...
    const {
        orgUnit,
        error,
    } = useRulesEngineOrgUnit(orgUnitId);
    ...
    const onViewAll = (stageId: string) => {
        history.push(`/enrollment/stageEvents?${buildUrlQueryString({
            orgUnitId,
            programId,
            stageId,
        })}`);
    ...
    const onAddNew = () => {
        history.push(`/new?${buildUrlQueryString({
            orgUnitId,
            programId,
            teiId,
        })}`);
    ...
    const onEnrollmentError = (message: string) => dispatch(showEnrollmentError({
        message,
    }));
