...
import { openNewEventInDataEntry, resetDataEntry } from '../actions/dataEntryLoad.actionBatchs';
import {
...
import { resetList } from '../../../../List/list.actions';
  ...
  action$.pipe(
    ...
    map(() => {
      ...
      if (metadataContainer.error) {
        log.error(
          errorCreator(errorMessages.PROGRAM_OR_STAGE_NOT_FOUND)({
...
const runRulesForNewSingleEvent = (
  ...
  if (metadataContainer.error) {
    ...
    rulesActions = getRulesActionsForEvent(metadataContainer.program, foundation, formId, orgUnit);
  } else {
    ...
    const currentEventMainData = getCurrentClientMainData(
      state,
      itemId,
      dataEntryId,
      {},
  ...
  action$.pipe(
    ...
    map((actionBatch) =>
      actionBatch.payload.find(
        (action) => action.type === newEventDataEntryActionTypes.START_RUN_RULES_ON_UPDATE,
      ),
    ),
    map((action) => {
