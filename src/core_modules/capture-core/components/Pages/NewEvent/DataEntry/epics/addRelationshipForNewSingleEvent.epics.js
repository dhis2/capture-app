  ...
  action$.pipe(
    ...
    map((action) => {
      ...
      if (meta.relationshipData) {
        ...
        return saveNewEventRelationships(
          relationshipData,
          action.meta.selections,
          action.meta.triggerAction,
        );
      }
      if (
        meta.triggerAction === newEventDataEntryActionTypes.START_SAVE_AFTER_RETURNED_TO_MAIN_PAGE
      ) {
...
export const teiForNewEventRelationshipSavedEpic = (action$: InputObservable) =>
  action$.pipe(
    ofType(newEventDataEntryActionTypes.TEI_FOR_NEW_EVENT_RELATIONSHIPS_SAVED),
    map((action) => {
