...
import { convertFormToClient, convertClientToList } from '../../../../../converters';

const formToListConverterFn = pipe(convertFormToClient, convertClientToList);
...
class TeiRelationshipSearchResults extends React.Component<Props> {
  ...
  getItemActions = (itemProps: Object) => {
    ...
    return (
      <div className={classes.itemActionsContainer}>
        <Button color="primary" onClick={() => this.onAddRelationship(itemProps.item)}>
          {i18n.t('Link')}
        </Button>
      </div>
    );
  ...
  render() {
    return <div>{this.props.resultsLoading ? <LoadingMask /> : this.renderResults()}</div>;
  }
}
