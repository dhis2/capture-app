...
import { batchActionTypes } from './teiRelationship.actions';
...
export const openRelationshipTeiSearchEpic = (action$: InputObservable, store: ReduxStore) =>
  action$.pipe(
    ...
    map(() => {
      ...
      return batchActions(
        [...addFormDataActions, initializeTeiSearch(searchId, programId, trackedEntityTypeId)],
...
export const TeiRelationshipNewOrEditSearchEpic = (action$: InputObservable) =>
  action$.pipe(
    ofType(teiSearchActionTypes.TEI_NEW_SEARCH, teiSearchActionTypes.TEI_EDIT_SEARCH),
    filter((action) => action.payload.searchId && action.payload.searchId === searchId),
    map(() => unsetSearching()),
  );
