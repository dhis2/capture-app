...
const CardListWithLoadingIndicator = withLoadingIndicator(
  null,
  null,
...
class ReviewDialogContents extends React.Component<Props> {
  ...
  getLinkButton = (itemProps: Object) => {
    ...
    return (
      <div className={classes.linkButtonCotainer}>
        <Button
          onClick={() => {
            onLink(id, values);
          }}
  ...
  render() {
    ...
    return (
      <>
        <DialogContent>
          <DialogTitle>{i18n.t('Possible duplicates found')}</DialogTitle>
          <div
            ref={(instance) => {
              this.contentListInstance = instance;
            }}
            style={divStyle}
          >
