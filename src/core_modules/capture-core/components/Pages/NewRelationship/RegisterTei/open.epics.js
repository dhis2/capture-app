...
import { initializeRegisterTei, initializeRegisterTeiFailed } from './registerTei.actions';
import {
...
function getTrackerProgram(suggestedProgramId: string) {
  ...
  try {
    ...
    log.error(errorCreator('tracker program for id not found')({ suggestedProgramId, error }));
...
export const openNewRelationshipRegisterTeiEpic = (action$: InputObservable, store: ReduxStore) =>
  action$.pipe(
    ...
    filter(
      (action) => action.payload.findMode && action.payload.findMode === findModes.TEI_REGISTER,
    ),
    switchMap(() => {
      ...
      try {
        ...
        log.error(errorCreator('TET for id not found')({ trackedEntityTypeId, error }));
        return Promise.resolve(
          initializeRegisterTeiFailed(i18n.t('Metadata error. see log for details')),
        );
      }
