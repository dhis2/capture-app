...
class UniqueTEADuplicateErrorMessageCreator extends React.Component<Props> {
  static renderPrerequisitesError() {
    return (
      <div>{i18n.t('A duplicate exists (but there were some errors, see log for details')}</div>
    );
  }
  ...
  static renderAttributeValueExistsUnsaved(attributeName: string) {
    ...
    return (
      <div>
        {i18n.t('You have already registered this {{attributeName}}', {
          attributeName: attributeNameLC,
        })}
      </div>
    );
  ...
  static renderTypeDifference(attributeName: string, trackedEntityTypeName: string) {
    ...
    return (
      <div>
        {i18n.t('A {{trackedEntityTypeName}} with this {{attributeName}} is already registered', {
  ...
  renderDefault(attributeName: string, trackedEntityTypeName: string, linkButtonClass: string) {
    ...
    return (
      <>
        ...
        <div>
          <LinkButton onClick={this.handleShowExisting} className={linkButtonClass}>
            {i18n.t('Show registered {{trackedEntityTypeName}}', {
              trackedEntityTypeName: trackedEntityTypeNameLC,
            })}
          </LinkButton>
        </div>
  ...
  renderContents() {
    ...
    if (errorData.tetId !== trackedEntityType.id) {
      return UniqueTEADuplicateErrorMessageCreator.renderTypeDifference(
        attributeName,
  ...
  render() {
    ...
    return <div className={classes.container}>{this.renderContents()}</div>;
  }
}
