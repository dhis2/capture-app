  ...
  class MainButtonHOC extends React.Component<Props> {
    static getButtonText(duplicatesFound: boolean, trackedEntityTypeName: string) {
      ...
      return duplicatesFound
        ...
        : i18n.t('Create {{trackedEntityTypeName}} and link', {
    ...
    renderButton() {
      ...
      return (
        <Button dataTest="dhis2-capture-create-and-link-button" primary onClick={onSave}>
          {MainButtonHOC.getButtonText(
            possibleDuplicatesFound,
            teiRegistrationMetadata.trackedEntityType.name,
          )}
        </Button>
      );
    }
    ...
    render() {
      const { onSave, possibleDuplicatesFound, ...passOnProps } = this.props;
      const mainButton = this.renderButton();
      return (
        // $FlowFixMe[cannot-spread-inexact] automated comment
        <InnerComponent mainButton={mainButton} {...passOnProps} />
      );
    }
  };
