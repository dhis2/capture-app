...
export const preCleanData = (cleanTemplates: boolean, listId: string) =>
  actionCreator(actionTypes.DATA_PRE_CLEAN)({ cleanTemplates, listId });

export const fetchTemplates = (programId: string, listId: string) =>
  actionCreator(actionTypes.TEMPLATES_FETCH)({ programId, listId });
...
export const initEventList = (
  ...
  context: Object,
  meta: {| listId: string, defaultConfig: Map<string, Object> |},
) => actionCreator(actionTypes.EVENT_LIST_INIT)({ selectedTemplate, context, ...meta });

export const initEventListSuccess = (listId: string, data: Object) =>
  actionCreator(actionTypes.EVENT_LIST_INIT_SUCCESS)({ ...data, listId });

export const initEventListError = (listId: string, errorMessage: string) =>
  actionCreator(actionTypes.EVENT_LIST_INIT_ERROR)({ listId, errorMessage });

export const initEventListCancel = (listId: string) =>
  actionCreator(actionTypes.EVENT_LIST_INIT_CANCEL)({ listId });

export const updateEventList = (listId: string, queryArgs: Object) =>
  actionCreator(actionTypes.EVENT_LIST_UPDATE)({ listId, queryArgs });

export const updateEventListSuccess = (listId: string, data: Object) =>
  actionCreator(actionTypes.EVENT_LIST_UPDATE_SUCCESS)({ ...data, listId });

export const updateEventListError = (listId: string, errorMessage: string) =>
  actionCreator(actionTypes.EVENT_LIST_UPDATE_ERROR)({ listId, errorMessage });

export const updateEventListCancel = (listId: string) =>
  actionCreator(actionTypes.EVENT_LIST_UPDATE_CANCEL)({ listId });

export const deleteEventSuccess = (eventId: string, listId: string) =>
  actionCreator(actionTypes.EVENT_DELETE_SUCCESS)({ eventId, listId });

export const deleteEventError = () => actionCreator(actionTypes.EVENT_DELETE_ERROR)();

export const unloadingContext = (listId: string) =>
  actionCreator(actionTypes.CONTEXT_UNLOADING)({ listId });
