...
export const WorkingListsContextBuilder = (props: Props) => {
  ...
  const dirtyTemplates = useMemo(
    () =>
      allTemplates
        ? allTemplates.some(
            (template) => template.updating || template.notPreserved || template.deleted,
          )
        : false,
    [allTemplates],
  );
  ...
  const loadedContextDefined = useMemo(() => loadedContext || {}, [loadedContext]);
  ...
  const templates = useMemo(() => allTemplates && allTemplates.filter((t) => !t.deleted), [
    allTemplates,
  ]);

  useEffect(() => () => onUnloadingContext && onUnloadingContext(), [onUnloadingContext]);
