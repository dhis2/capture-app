...
import { updateEventListSuccess, updateEventListError } from '../workingLists.actions';
...
export const updateEventWorkingListAsync = (
  ...
  const sourceQueryData = getSourceQueryArgsForUpdateWorkingList(
    state,
    listId,
    sourceQueryArgsPart,
  );
  ...
  return getEventWorkingListDataAsync(
    buildQueryArgs(sourceQueryData, {
      listId,
    ...
    .then((data) => updateEventListSuccess(listId, data))
    .catch((error) => {
