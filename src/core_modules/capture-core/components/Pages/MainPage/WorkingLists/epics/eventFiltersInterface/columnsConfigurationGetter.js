...
const getCustomConfig = (
  ...
  customOrder.reduce((acc, elementKey) => {
...
const getCustomColumnsConfiguration = (defaultConfig, customOrder) => {
  const defaultConfigOrderAsHashMapById = defaultConfig.reduce((acc, orderElement) => {
    acc.set(orderElement.id, orderElement);
    return acc;
  }, new Map());

  const defaultConfigOrderAsHashMapByConfigName = defaultConfig.reduce((acc, orderElement) => {
  ...
  if (customConfigAsHashMap.size > 0) {
    ...
    return [...customConfigAsHashMap.values(), ...otherConfigElements];
  }
