...
export const updateTemplateEpic = (action$: InputObservable, store: ReduxStore) =>
  action$.pipe(
    ...
    concatMap((action) => {
      const { template, eventQueryCriteria, programId, listId } = action.payload;
...
export const addTemplateEpic = (action$: InputObservable, store: ReduxStore) =>
  action$.pipe(
    ofType(actionTypes.TEMPLATE_ADD),
    concatMap((action) => {
      const { name, eventQueryCriteria, clientId, programId, listId } = action.payload;
