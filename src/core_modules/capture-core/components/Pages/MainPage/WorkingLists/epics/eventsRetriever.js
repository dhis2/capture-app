...
const getMainColumns = (columnsOrder: Array<ColumnConfig>) =>
  columnsOrder.reduce((accMainColumns, column) => {
...
const getApiFilterQueryArgument = (
  filters: ?{ [id: string]: string },
  mainColumns: { [id: string]: Object },
) => {
  ...
  const filterArgument =
    filterQueries && filterQueries.length > 0 ? { filter: filterQueries } : null;
...
const getEventDateQueryArgs = (filter: Array<string> | string) => {
  ...
  if (Array.isArray(filter)) {
    eventDateQueryArgs = filter.reduce((accEventDateQueryArgs, filterPart: string) => {
...
const getMainApiFilterQueryArguments = (
  filters: ?{ [id: string]: string },
  mainColumns: { [id: string]: Object },
) => {
...
const getApiOrderByQueryArgument = (
  sortById: string,
...
const createApiQueryArgs = (queryArgs: Object, mainColumns: Object) => {
  ...
  const apiQueryArgsWithServerPropName = Object.keys(apiQueryArgs).reduce(
    (accApiQueryArgs, key) => {
