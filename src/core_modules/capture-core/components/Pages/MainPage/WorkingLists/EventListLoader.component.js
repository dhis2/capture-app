...
const EventListLoader = (props: Props) => {
  const { currentTemplate, listId, defaultConfig, programId, ...passOnProps } = props;
  ...
  const hasContextChanged = React.useMemo(
    () => !onCheckSkipReload(programId, orgUnitId, categories, lastTransaction, listContext),
    [onCheckSkipReload, programId, orgUnitId, categories, lastTransaction, listContext],
  );
  ...
  const ready =
    ...
    (!prevTemplateRef.current ||
      currentTemplate.id === prevTemplateRef.current.id ||
      !!currentTemplate.skipInitDuringAddProcedure) &&
    (!prevListIdRef.current || prevListIdRef.current === listId) &&
    (!dirtyEventList || !firstRunRef.current) &&
    !eventListIsLoading;
