...
import type { ApiEventQueryCriteria, ColumnConfig } from './workingLists.types';

const determinePrimitiveConfigValue = (value, nextValue) =>
...
const isCurrentListModified = (
  ...
  const columns = columnOrder.filter((c) => c.visible).map((c) => c.id);
...
const EventListConfig = (props: Props) => {
  ...
  const { next: nextMeta = {}, initial: initialMeta, nextInitial: nextInitialMeta, ...mainMeta } =
    listMeta || {};

  const { filters, sortById, sortByDirection, currentPage, rowsPerPage } = mainMeta;
  ...
  const currentListIsModified = calcInitialMeta
    ? isCurrentListModified(
        calcFilters,
        calcSortById,
        calcSortByDirection,
        columnOrder,
  ...
  const updateTemplateHandler = React.useCallback(
    (template, listConfig) => {
      const eventQueryCriteria: ApiEventQueryCriteria = convertToEventQueryCriteria(listConfig);
  ...
  const deleteTemplateHandler = React.useCallback(
    (template, listId) => onDeleteTemplate(template, programId, listId),
    [onDeleteTemplate, programId],
  );
