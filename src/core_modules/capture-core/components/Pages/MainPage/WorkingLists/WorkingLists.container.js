...
const mapStateToProps: MapStateToPropsFactory = (state: ReduxState, props: { listId: string }) => {
  ...
  return {
    currentTemplate:
      state.workingListsTemplates[listId] &&
      state.workingListsTemplates[listId].selectedTemplateId &&
      state.workingListsTemplates[listId].templates &&
      state.workingListsTemplates[listId].templates.find(
        (template) => template.id === state.workingListsTemplates[listId].selectedTemplateId,
      ),
    templates: state.workingListsTemplates[listId] && state.workingListsTemplates[listId].templates,
    templatesForProgramId:
      state.workingListsTemplates[listId] && state.workingListsTemplates[listId].programId,
    templatesAreLoading:
      !!state.workingListsTemplates[listId] && !!state.workingListsTemplates[listId].loading,
    loadTemplatesError:
      state.workingListsTemplates[listId] && state.workingListsTemplates[listId].loadError,
    loadEventListError:
      state.workingListsUI[listId] && state.workingListsUI[listId].dataLoadingError,
    ...
    eventListIsUpdating:
      !!state.workingListsUI[listId] && !!state.workingListsUI[listId].isUpdating,
    eventListIsUpdatingWithDialog:
      !!state.workingListsUI[listId] && !!state.workingListsUI[listId].isUpdatingWithDialog,
    lastEventIdDeleted:
      state.workingListsUI[listId] && state.workingListsUI[listId].lastEventIdDeleted,
  };
};
