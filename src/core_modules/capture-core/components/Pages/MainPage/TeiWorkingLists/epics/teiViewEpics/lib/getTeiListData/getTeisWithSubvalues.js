...
import type { SubvalueKeysByType, SubvaluesByType } from './getTeisWithSubvalues.types';
...
const subvalueGetterByType: {|[string]: any |} = {
    [dataElementTypes.ORGANISATION_UNIT]: async (keys: Array<string>) => {
        ...
        const { organisationUnits = [] } = await getApi()
...
const subvaluePostProcessorByType: {|[string]: any |} = {
    [dataElementTypes.IMAGE]: ({
        ...
        return {
            name,
            value,
            url: `${baseUrl}/trackedEntityInstances/${teiId}/${columnId}/image`,
        };
    },
...
export const getTeisWithSubvalues = async (clientTeis: ClientTeis, columnsMetaForDataFetching: TeiColumnsMetaForDataFetchingArray) => {
