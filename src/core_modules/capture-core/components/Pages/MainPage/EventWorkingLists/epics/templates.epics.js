...
export const retrieveTemplatesEpic = (action$: InputObservable) =>
  action$.pipe(
    ...
    filter(
      ({ payload: { workingListsType } }) => workingListsType === SINGLE_EVENT_WORKING_LISTS_TYPE,
    ),
...
export const updateTemplateEpic = (action$: InputObservable, store: ReduxStore) =>
  action$.pipe(
    ...
    concatMap((action) => {
      const { template, criteria: eventQueryCriteria, programId, storeId } = action.payload;
      ...
      const requestPromise = api
        ...
        .then(() => {
          ...
          return updateTemplateSuccess(template.id, eventQueryCriteria, {
            storeId,
            isActiveTemplate,
          });
        })
        .catch((error) => {
          ...
          const isActiveTemplate =
            store.value.workingListsTemplates[storeId].selectedTemplateId === template.id;
          return updateTemplateError(template.id, eventQueryCriteria, {
            storeId,
            isActiveTemplate,
          });
        });
...
export const addTemplateEpic = (action$: InputObservable, store: ReduxStore) =>
  action$.pipe(
    ...
    concatMap((action) => {
      const { name, criteria: eventQueryCriteria, clientId, programId, storeId } = action.payload;
      ...
      const requestPromise = api
        ...
        .then((result) => {
          ...
          return api
            ...
            .then(() => {
              ...
              return addTemplateSuccess(result.response.uid, clientId, {
                storeId,
                isActiveTemplate,
              });
            });
        ...
        .catch((error) => {
          ...
          const isActiveTemplate =
            store.value.workingListsTemplates[storeId].selectedTemplateId === clientId;
          return addTemplateError(clientId, {
            storeId,
            isActiveTemplate,
          });
        });
