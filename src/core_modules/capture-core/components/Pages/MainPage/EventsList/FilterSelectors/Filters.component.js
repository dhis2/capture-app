...
const renderIndividualFilterButtons = (
  ...
  individualElementsArray.map((element) => (
    <div
      key={element.id}
...
const Filters = (props: Props) => {
  const { columns, stickyFilters, listId, onRestMenuItemSelected, classes } = props;
  ...
  const handleRestMenuItemSelected = React.useCallback(
    ...
    [setVisibleSelector, onRestMenuItemSelected],
  );
  ...
  const filterButtons = React.useMemo(() => {
    ...
    const restButton = renderRestButton(restElementsArray, listId, handleRestMenuItemSelected);
