...
class FilterSelectorContents extends React.PureComponent<Props> {
  static getOptionSetComponent() {
    return withButtons()(withData()(OptionSetFilter));
  }
  ...
  getContentsComponent() {
    ...
    if (
      optionSet &&
      optionSet.options &&
      optionSet.options.length <= MAX_OPTIONS_COUNT_FOR_OPTION_SET_CONTENTS
    ) {
    ...
    if (FilterSelectorContents.hasStylesHOC.includes(type)) {
      return withButtons()(withData()(SelectorContent));
    }
    return withButtons()(withData()(SelectorContent));
  }
