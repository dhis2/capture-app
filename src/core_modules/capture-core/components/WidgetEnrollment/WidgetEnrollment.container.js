...
const useCanAddNew = (enrollments, programId) =>
    useMemo(
        () => enrollments.filter(item => item.program === programId).every(item => item.status !== plainStatus.ACTIVE),
...
export const WidgetEnrollment = ({
    ...
    const canAddNew = useCanAddNew(enrollments, programId);
