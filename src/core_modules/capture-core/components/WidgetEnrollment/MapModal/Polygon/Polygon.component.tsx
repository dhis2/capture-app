...
const coordsToFeatureCollection = (inputCoordinates: any): FeatureCollection | null => {
    ...
    const list = inputCoordinates[0].length > 2 ? inputCoordinates[0] : inputCoordinates.map((c: [number, number]) => [c[1], c[0]]);
...
const PolygonPlain = ({
    ...
    const [polygonArea, setPolygonArea] = useState<number[][] | undefined>(defaultValues);
    ...
    const onMapPolygonCreated = (e: any) => {
        const polygonCoordinates = e.layer.toGeoJSON().geometry.coordinates[0].map((c: [number, number]) => [c[1], c[0]]);
            ...
            {drawingState && (
                <>
                    <Button
                        ...
                        secondary
                    >
                        {String(i18n.t('Close without saving'))}
                    </Button>
                    <ConditionalTooltip
                        content={String(i18n.t('Finish drawing before saving'))}
                    ...
                    >
                        <Button
                            ...
                            onClick={() => {
                                ...
                                onSetCoordinates(convertedCoordinates || null as any);
                                setOpen(false);
                            }}
                            primary
                        >
                            {String(i18n.t('Set area'))}
                        </Button>
                    </ConditionalTooltip>
                </>
            )}
    ...
    return (
        <Modal large>
            <ModalTitle>{String(i18n.t('Area'))}</ModalTitle>
...
export const Polygon = withStyles(styles)(PolygonPlain) as ComponentType<Omit<PolygonProps, 'classes'>>;
