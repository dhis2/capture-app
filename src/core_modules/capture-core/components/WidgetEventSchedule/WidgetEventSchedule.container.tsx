...
export const WidgetEventSchedule = ({
    ...
    const { programStageScheduleConfig } = useScheduleConfigFromProgramStage(stageId) as { programStageScheduleConfig: { displayDueDateLabel?: string } };
    ...
    const suggestedScheduleDate = useDetermineSuggestedScheduleDate({
        ...
        initialScheduleDate: initialScheduleDate as string,
        ...passOnProps,
    });
    ...
    const [scheduledOrgUnit, setScheduledOrgUnit] = useState<{ id: string, name: string, path?: string } | undefined>();
    const [validation, setValidation] = useState<{ error: boolean, validationText: string } | undefined>();
    ...
    const convertScheduleDate = (date: string | undefined, validationResult: { error: boolean } = { error: false }) => {
    ...
    const [categoryOptionsError, setCategoryOptionsError] = useState<Record<string, { touched: boolean, valid: boolean }> | undefined>();
        ...
        if (programCategory?.categories &&
            ...
            const errors = programCategory.categories
                ...
                .reduce((acc, category) => {
                    ...
                    return acc;
                }, {} as Record<string, { touched: boolean, valid: boolean }>);
    ...
    return (
        <WidgetEventScheduleComponent
            ...
            onCancel={onCancel}
            setScheduleDate={date => setScheduleDate(date || '')}
            ...
            orgUnit={scheduledOrgUnit || { id: '', name: '' }}
            notes={notes}
