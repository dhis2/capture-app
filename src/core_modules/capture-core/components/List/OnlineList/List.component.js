...
class List extends React.Component<Props> {
  ...
  getCustomEndCellHeader = () => {
    const {
      getCustomEndCellHeader,
      getCustomEndCellBody,
      customEndCellHeaderStyle,
      classes,
    } = this.props;
  ...
  renderHeaderRow(visibleColumns: Array<Column>) {
    ...
    const headerCells = visibleColumns.map((column, index) => (
      <HeaderCell
        ...
        style={{
          width:
            this.props.isUpdating && this.columnHeaderInstances.length - 1 >= index
        ...
        {dataSource.map((row) => {
          const cells = visibleColumns.map((column) => (
            <Cell key={column.id} className={classNames(classes.cell, classes.bodyCell)}>
              <div
                style={
                  List.typesWithRightPlacement.includes(column.type) ? { textAlign: 'right' } : null
                }
              >
                {row[column.id]}
              </div>
  ...
  render() {
    ...
    return (
      <div className={classes.tableContainer}>
        <Table className={classes.table} data-test="dhis2-capture-event-list-table">
          <Head>{this.renderHeaderRow(visibleColumns)}</Head>
          <Body data-test="dhis2-capture-event-list-body">{this.renderBody(visibleColumns)}</Body>
        </Table>
      </div>
    );
  }
