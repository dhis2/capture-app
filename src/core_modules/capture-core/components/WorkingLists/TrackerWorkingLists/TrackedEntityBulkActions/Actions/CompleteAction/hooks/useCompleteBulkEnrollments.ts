    ...
    const {
        ...
        ({ enrollments }: any) => importValidEnrollments({ dataEngine, enrollments }),
        {
            ...
            onError: (serverResponse, variables) => {
                removeQueries();
                showAlert({ message: tCustomTerm('An error occurred when completing the {{enrollmentsLabel}}', { enrollmentsLabel }) }); // eslint-disable-line max-len
                log.error(
        ...
        }),
        {
            ...
            onError: (serverResponse: any, { enrollments }: any) => {
                ...
                if (!errors) {
                    ...
                    showAlert({ message: tCustomTerm('An unknown error occurred when completing {{enrollmentsLabel}}', { enrollmentsLabel }) });
                    return;
                }
