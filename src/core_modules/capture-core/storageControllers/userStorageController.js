...
function createStorageController(mainStorageController: typeof StorageController) {
  const storageName = getStorageName(mainStorageController.name);
  const appCacheVersion = mainStorageController.version;
  const stores = getStores();
  const storageController = new StorageController(storageName, appCacheVersion, {
...
const storeSpecificCreateActions = {
  [userStores.CATEGORY_OPTIONS]: (objectStore) => {
    objectStore.createIndex('categoryId', 'categories', {
      multiEntry: true,
    });
  },
...
async function initUserControllerAsync(mainStorageController: typeof StorageController) {
  const userStorageController = createStorageController(mainStorageController);

  let upgradeTempData;
  await userStorageController.open(
    (storage) =>
      storage.get(userStores.REDUX_PERSIST, 'reduxPersist:offline').then((data) => {
    ...
    (storage) => {
      ...
      return storage.set(userStores.REDUX_PERSIST, upgradeTempData);
    },
    (objectStore, adapter) => {
