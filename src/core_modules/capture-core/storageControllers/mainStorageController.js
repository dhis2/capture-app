...
async function initMainControllerAsync(adapterTypes: Array<any>, onCacheExpired: Function) {
  const mainStorageController = createStorageController(
    MAIN_STORAGE_KEY,
    adapterTypes,
    onCacheExpired,
  );
  ...
  await mainStorageController.open(
    (storage) =>
      storage.get(mainStores.USER_CACHES, 'accessHistory').then((data) => {
    ...
    (storage) => {
      ...
      return storage.set(mainStores.USER_CACHES, upgradeTempData);
    },
  );
