...
function getStorageName(mainStorageName: string, userId: string): string {
...
const storeSpecificCreateActions: Record<string, (objectStore: IDBObjectStore) => void> = {
...
export async function initUserControllerAsync(mainStorageController: StorageController, currentUserId: string): Promise<StorageController> {
    const userStorageController =
        createStorageController(mainStorageController, currentUserId);

    let upgradeTempData: unknown;
