...
export default function convertToClientRelationship(
  serverRelationship: Object,
  relationshipTypes: Array<RelationshipType>,
) {
  const relationshipType = relationshipTypes.find(
    (r) => r.id === serverRelationship.relationshipType,
  );
  if (!relationshipType) {
    return null;
  }
  ...
  return {
    ...
    from: getClientConstraintByType[relationshipType.from.entity](
      serverRelationship.from,
      relationshipType.from,
    ),
    to: getClientConstraintByType[relationshipType.to.entity](
      serverRelationship.to,
      relationshipType.to,
    ),
  };
}
