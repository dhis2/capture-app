...
import { actionTypes as teiSearchActionTypes } from '../../components/TeiSearch/actions/teiSearch.actions';

import { actionTypes as teiSearchOrgUnitActionTypes } from '../../components/TeiSearch/SearchOrgUnitSelector/searchOrgUnitSelector.actions';
...
export const teiSearchDesc = createReducerDescription(
  {
    ...
    [teiSearchActionTypes.SEARCH_TEI_RESULT_RETRIEVED]: (state, action) => {
      ...
      const teis = data.trackedEntityInstanceContainers
        ? data.trackedEntityInstanceContainers.map((t) => ({ id: t.id, values: t.values }))
        : [];
      return {
        ...state,
        [searchId]: {
