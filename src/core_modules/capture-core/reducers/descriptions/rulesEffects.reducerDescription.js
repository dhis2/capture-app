...
import type { OutputEffect, GeneralErrorEffect, GeneralWarningEffect } from '../../rules/engine';
...
export const rulesEffectsHiddenFieldsDesc = createReducerDescription(
  {
    [actionTypes.UPDATE_RULES_EFFECTS]: (state, action) => {
      ...
      const hideEffects: { [id: string]: Array<OutputEffect> } =
        action.payload.rulesEffects && action.payload.rulesEffects[effectActions.HIDE_FIELD];
...
export const rulesEffectsCompulsoryFieldsDesc = createReducerDescription(
  {
    [actionTypes.UPDATE_RULES_EFFECTS]: (state, action) => {
      ...
      const compulsoryEffects: { [id: string]: Array<OutputEffect> } =
        action.payload.rulesEffects && action.payload.rulesEffects[effectActions.MAKE_COMPULSORY];
...
export const rulesEffectsHiddenSectionsDesc = createReducerDescription(
  {
    [actionTypes.UPDATE_RULES_EFFECTS]: (state, action) => {
      ...
      const hideEffects: { [id: string]: Array<OutputEffect> } =
        action.payload.rulesEffects && action.payload.rulesEffects[effectActions.HIDE_SECTION];
      newState[action.payload.formId] = hideEffects
        ? Object.keys(hideEffects).reduce((accState, key) => {
            accState[key] = true;
            return accState;
...
export const rulesEffectsFeedbackDesc = createReducerDescription(
  {
    [actionTypes.UPDATE_RULES_EFFECTS]: (state, action) => {
      ...
      const displayTextEffects: { [id: string]: Array<OutputEffect> } =
        action.payload.rulesEffects && action.payload.rulesEffects[effectActions.DISPLAY_TEXT];
      const displayKeyValuePairEffects: {
        [id: string]: Array<OutputEffect>,
      } =
        action.payload.rulesEffects &&
      ...
      newState[action.payload.formId] = {
        displayTexts:
          displayTextEffects && displayTextEffects.feedback
            // $FlowFixMe[prop-missing] automated comment
            ? displayTextEffects.feedback.map((e) => e.displayText)
            : null,
        displayKeyValuePairs:
          displayKeyValuePairEffects && displayKeyValuePairEffects.feedback
            // $FlowFixMe[prop-missing] automated comment
            ? displayKeyValuePairEffects.feedback.map((e) => e.displayKeyValuePair)
            : null,
      };
...
export const rulesEffectsIndicatorsDesc = createReducerDescription(
  {
    [actionTypes.UPDATE_RULES_EFFECTS]: (state, action) => {
      ...
      newState[action.payload.formId] = {
        displayTexts:
          displayTextEffects && displayTextEffects.indicators
            // $FlowFixMe[prop-missing] automated comment
            ? displayTextEffects.indicators.map((e) => e.displayText)
            : null,
        displayKeyValuePairs:
          displayKeyValuePairEffects && displayKeyValuePairEffects.indicators
            // $FlowFixMe[prop-missing] automated comment
            ? displayKeyValuePairEffects.indicators.map((e) => e.displayKeyValuePair)
            : null,
      };
...
export const rulesEffectsGeneralErrorsDesc = createReducerDescription(
  {
    [actionTypes.UPDATE_RULES_EFFECTS]: (state, action) => {
      ...
      const errorEffects: { [id: string]: Array<GeneralErrorEffect> } =
        action.payload.rulesEffects && action.payload.rulesEffects[effectActions.SHOW_ERROR];
      const errorEffectsOnComplete: {
        [id: string]: Array<GeneralErrorEffect>,
      } =
        action.payload.rulesEffects &&
        action.payload.rulesEffects[effectActions.SHOW_ERROR_ONCOMPLETE];

      const generalErrors =
        errorEffects && errorEffects.general ? errorEffects.general.map((e) => e.error) : null;
      const generalErrorsOnComplete =
        errorEffectsOnComplete && errorEffectsOnComplete.general
          ? errorEffectsOnComplete.general.map((e) => e.error)
          : null;

      newState[action.payload.formId] = {
        [mapMessageEffectTypeToStateKey[effectActions.SHOW_ERROR]]: generalErrors,
        [mapMessageEffectTypeToStateKey[
          effectActions.SHOW_ERROR_ONCOMPLETE
        ]]: generalErrorsOnComplete,
      };
...
export const rulesEffectsGeneralWarningsDesc = createReducerDescription(
  {
    [actionTypes.UPDATE_RULES_EFFECTS]: (state, action) => {
      ...
      const warningsEffects: {
        [id: string]: Array<GeneralWarningEffect>,
      } = action.payload.rulesEffects && action.payload.rulesEffects[effectActions.SHOW_WARNING];
      const warningsEffectsOnComplete: {
        [id: string]: Array<GeneralWarningEffect>,
      } =
        action.payload.rulesEffects &&
        action.payload.rulesEffects[effectActions.SHOW_WARNING_ONCOMPLETE];
      ...
      const generalWarnings =
        warningsEffects && warningsEffects.general
          ? warningsEffects.general.map((w) => w.warning)
          : null;
      const generalWarningsOnComplete =
        warningsEffectsOnComplete && warningsEffectsOnComplete.general
          ? warningsEffectsOnComplete.general.map((w) => w.warning)
          : null;

      newState[action.payload.formId] = {
        [mapMessageEffectTypeToStateKey[effectActions.SHOW_WARNING]]: generalWarnings,
        [mapMessageEffectTypeToStateKey[
          effectActions.SHOW_WARNING_ONCOMPLETE
        ]]: generalWarningsOnComplete,
      };
