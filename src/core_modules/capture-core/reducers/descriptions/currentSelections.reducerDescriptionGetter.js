...
import { actionTypes as quickSelectorActionTypes } from '../../components/LockedSelector/QuickSelector/actions/QuickSelector.actions';
...
import { actionTypes as crossPageActionTypes } from '../../components/Pages/actions/crossPage.actions';
...
export const getCurrentSelectionsReducerDesc = (appUpdaters: Updaters) =>
  createReducerDescription(
    {
      ...
      [editEventActionTypes.EVENT_FROM_URL_RETRIEVED]: (state, action) => {
        ...
        const categories = categoriesData
          ? categoriesData.reduce((acc, data) => {
              acc[data.categoryId] = data.categoryOption.id;
              return acc;
            }, {})
          : undefined;

        const categoriesMeta = categoriesData
          ? categoriesData.reduce((acc, data) => {
              acc[data.categoryId] = {
      ...
      [crossPageActionTypes.AFTER_SETTING_ORG_UNIT_DO_CATEGORIES_RESET]: (state, action) => {
        ...
        const container = resetCategories.reduce(
          (acc, categoryId) => {
