...
export default class OptionSet {
  ...
  constructor(
    ...
    this._options = !options
      ? []
      : options.reduce((accOptions: Array<Option>, currentOption: Option) => {
  ...
  set inputType(inputType: ?string) {
    ...
    if (inputTypesAsArray.includes(inputType)) {
      ...
      log.warn(
        errorCreator(OptionSet.errorMessages.UNSUPPORTED_INPUTTYPE)({
          optionSet: this,
          inputType,
        }),
      );
  ...
  set viewType(viewType: ?string) {
    ...
    if (viewTypes[viewType]) {
      ...
      log.warn(
        errorCreator(OptionSet.errorMessages.UNSUPPORTED_VIEWTYPE)({
          optionSet: this,
          viewType,
        }),
      );
  ...
  getOptionThrowIfNotFound(value: Value): Option {
    ...
    if (!option) {
      throw new Error(
        errorCreator(OptionSet.errorMessages.OPTION_NOT_FOUND)({
          OptionSet: this,
          value,
        }),
      );
