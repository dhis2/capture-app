...
const getUpdaterFoundFn = (reducerDescription: ReducerDescription) => (
  state: any,
  action: Action,
) => {
  log.trace(
...
const getUpdaterExecutedFn = (reducerDescription: ReducerDescription) => (
  state: any,
  action: Action,
) => {
  log.trace(
...
const getProductionReducer = (reducerDescription: ReducerDescription) => (
  state: any,
  action: Action,
) =>
  updateStatePartInProduction(
    state,
    action,
...
const getDevelopmentReducer = (reducerDescription: ReducerDescription) => {
  ...
  return (state: any, action: Action) => {
    ...
    log.trace(
      `reducer ${reducerDescription.name} starting. Action is: ${JSON.stringify(logAction)}`,
    );
...
export function buildReducersFromDescriptions(reducerDescriptions: Array<ReducerDescription>) {
  const reducers = reducerDescriptions.reduce(
    (
      accReducers: { [reducerName: string]: Reducer<any, Action> },
