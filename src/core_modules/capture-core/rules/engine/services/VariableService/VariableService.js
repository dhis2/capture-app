...
export default class VariableService {
  static getDataElementValueForVariable(
    value: any,
  ...
  constructor(onProcessValue: (value: any, type: $Values<typeof typeKeys>) => any) {
    ...
    this.mapSourceTypeToGetterFn = {
      ...
      [variableSourceTypes.DATAELEMENT_NEWEST_EVENT_PROGRAM_STAGE]: this
  ...
  getVariables(
    ...
    const variables = programVariables.reduce((accVariables, programVariable) => {
      ...
      if (!getterFn) {
        log.error(
          `Unknown programRuleVariableSourceType:${programVariable.programRuleVariableSourceType}`,
        );
      ...
      if (variableSourceTypesDataElementSpecific[sourceType]) {
        ...
        variable = this.preCheckTrackedEntitySpecificSourceType(
          programVariable,
          trackedEntityAttributes,
        );
      }
      ...
      if (!variable) {
        // run post getter
        if (variableSourceTypesDataElementSpecific[sourceType]) {
          variable = this.postGetVariableForDataElementSpecificSourceType(
            programVariable,
    ...
    const variablesWithContextVariables = {
      ...variables,
    ...
    const variablesWithContextAndConstantVariables = {
      ...
      ...this.getConstantVariables(programRulesContainer.constants),
    };
  ...
  preCheckDataElementSpecificSourceType(
    programVariable: ProgramRuleVariable,
    dataElements: ?DataElements,
  ) {
  ...
  preCheckTrackedEntitySpecificSourceType(
    ...
    const attribute =
      attributeId && trackedEntityAttributes && trackedEntityAttributes[attributeId];
    if (!attribute) {
      log.warn(
        `Variable id:${programVariable.id} name:${
          programVariable.displayName
  ...
  postGetVariableForDataElementSpecificSourceType(
    programVariable: ProgramRuleVariable,
    dataElements: DataElements,
  ) {
    const { dataElementId } = programVariable;
  ...
  getVariableForSelectedEntityAttributes(
    ...
    const attribute: TrackedEntityAttribute =
      // $FlowFixMe[incompatible-use] automated comment
      // $FlowFixMe[incompatible-type] automated comment
      sourceData.trackedEntityAttributes[trackedEntityAttributeId];
    // $FlowFixMe[incompatible-type] automated comment
    ...
    if (!hasValue) {
      return this.buildVariable(EMPTY_STRING, attribute.valueType, {
        variablePrefix: variablePrefixes.TRACKED_ENTITY_ATTRIBUTE,
        useNameForOptionSet: programVariable.useNameForOptionSet,
      });
    }
    ...
    return this.buildVariable(variableValue, attribute.valueType, {
      variablePrefix: variablePrefixes.TRACKED_ENTITY_ATTRIBUTE,
      useNameForOptionSet: programVariable.useNameForOptionSet,
    });
  }

  getVariableForCurrentEvent(
    programVariable: ProgramRuleVariable,
    sourceData: SourceData,
  ): ?RuleVariable {
    const { dataElementId } = programVariable;
    ...
    return this.buildVariable(value, dataElement.valueType, {
      variablePrefix: variablePrefixes.DATAELEMENT,
  ...
  getVariableForNewestEventProgramStage(
    ...
    const stageEvents =
      sourceData.eventsContainer && sourceData.eventsContainer.byStage[programStageId];
    if (!stageEvents) {
      return null;
    }
  ...
  getVariableForNewestEventProgram(
    programVariable: ProgramRuleVariable,
    sourceData: SourceData,
  ): ?RuleVariable {
  ...
  getVariableForPreviousEventProgram(
    programVariable: ProgramRuleVariable,
    sourceData: SourceData,
  ): ?RuleVariable {
    const events = sourceData.eventsContainer && sourceData.eventsContainer.all;
  ...
  getEventContextVariables(executingEvent: ?EventData, eventsContainer: ?EventsDataContainer) {
    ...
    if (executingEvent) {
      variables.event_date = this.buildVariable(executingEvent.eventDate, typeKeys.DATE, {
        variablePrefix: variablePrefixes.CONTEXT_VARIABLE,
      });

      variables.due_date = this.buildVariable(executingEvent.dueDate, typeKeys.DATE, {
        variablePrefix: variablePrefixes.CONTEXT_VARIABLE,
      });
  ...
  getEnrollmentContextVariables(selectedEnrollment: ?Enrollment) {
    ...
    if (selectedEnrollment) {
      ...
      variables.incident_date = this.buildVariable(selectedEnrollment.incidentDate, typeKeys.DATE, {
        variablePrefix: variablePrefixes.CONTEXT_VARIABLE,
      });
    }
