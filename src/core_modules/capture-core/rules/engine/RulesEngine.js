...
const convertRuleEffectDataToOutputBaseValue = (data: any, valueType: string) => {
  ...
  return ruleEffectDataConvertersByType[valueType]
    ? ruleEffectDataConvertersByType[valueType](data)
    : data;
};
 ...
 * @param variablesHash
 */
function updateVariableHashWhenActionIsAssignValue(
  content: string,
  action: string,
  data: any,
      ...
      variablesPresent.forEach((variablePresent) => {
        ...
        if (isDefined(variablesHash[variablePresent])) {
          ...
          expression = expression.replace(
            new RegExp(
              `${variablesHash[variablePresent].variablePrefix}\\{${variablePresent}\\}`,
              'g',
            ),
...
function getProgramRuleEffects(
  ...
  const processRulesEffects = getRulesEffectsProcessor(
      ...
      if (isProgramRuleExpressionEffective) {
        programRuleEffects = rule.programRuleActions.map(
          ({
            ...
            updateVariableHashWhenActionIsAssignValue(
              content,
              action,
              ruleEffectData,
              variablesHash,
...
export default class RulesEngine {
  ...
  static programRuleEffectsForTEI(
    ...
    const dhisFunctions = d2Functions(
      RulesEngine.dateUtils,
      RulesEngine.variableService(),
      variablesHash,
    );
