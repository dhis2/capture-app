...
function convertFormValuesToClientValues(
...
function getValidFormValues(
  formValues: { [key: string]: any },
  fieldsValidation: { [key: string]: boolean },
) {
  return Object.keys(formValues)
    .filter((key) => fieldsValidation[key])
    .reduce(
      (accValidFormValues, key) => ({
        ...accValidFormValues,
        [key]: formValues[key],
      }),
      {},
...
function getFieldsValidationForForm(sectionsFieldsUI: Object, formId: string) {
  const fieldsUIState = Object.keys(sectionsFieldsUI)
    ...
    .reduce((accFormElementsUIState, stateKey) => {
      accFormElementsUIState = {
        ...accFormElementsUIState,
        ...sectionsFieldsUI[stateKey],
      };
      return accFormElementsUIState;
    }, {});

  const fieldsValidation = Object.keys(fieldsUIState).reduce((accFieldsValidation, key) => {
...
export function getCurrentClientValues(
  ...
  const updatedCurrentFormData = updatedEventField
    ? {
        ...currentFormData,
        [updatedEventField.elementId]: updatedEventField.value,
      }
    : currentFormData;
  ...
  const updatedFieldValidations = updatedEventField
    ? {
        ...fieldValidations,
        [updatedEventField.elementId]: updatedEventField.valid,
      }
    : fieldValidations;
