...
async function getIdsToLoad(optionSetsOutline: Array<InputOutline>) {
  const idsToLoad = [];
  const { storageController, storeNames } = getContext();
  // $FlowFixMe[prop-missing] automated comment
  await optionSetsOutline.asyncForEach(async (outline) => {
    const storeOptionSet = await storageController.get(storeNames.OPTION_SETS, outline.id);
...
const getGroupsByOptionSet = (
  ...
  optionGroups: Array<ApiOptionGroup>,
) =>
  optionGroups.reduce((accGroupsByOptionSet, optionGroup) => {
...
const getGroupsBatchesByOptionSet = (optionGroupsBatches: Array<Array<ApiOptionGroup>>) =>
  optionGroupsBatches.reduce(
    (accGroupsByOptionSets, batch) => getGroupsByOptionSet(accGroupsByOptionSets, batch),
    {},
  );
...
const getCacheOptionSets = (
  optionSets: Array<ApiOptionSet>,
  optionGroupsByOptionSet: OptionGroupsByOptionSet,
) =>
  optionSets.map((optionSet) => ({
    ...
    options:
      optionSet.options &&
      [...optionSet.options.values()].map((option) => ({
        id: option.id,
    ...
    optionGroups:
      optionGroupsByOptionSet[optionSet.id] &&
      optionGroupsByOptionSet[optionSet.id].map((optionGroup) => ({
        id: optionGroup.id,
        displayName: optionGroup.displayName,
        options:
          optionGroup.options && [...optionGroup.options.values()].map((option) => option.id),
      })),
  }));

export async function loadOptionSets(optionSetsOutline: Array<InputOutline>) {
  const filteredOptionSetsOutline = optionSetsOutline.reduce(
    (accFilteredOptionSetsOutline, optionSetMeta) => {
  ...
  const optionSetsBatches = await Promise.all(
    batchedOptionSetIds.map((batch) => queryOptionSets(batch)),
  );

  const optionGroupsBatches = await Promise.all(
    batchedOptionSetIds.map((batch) => getOptionGroups(batch)),
  );
