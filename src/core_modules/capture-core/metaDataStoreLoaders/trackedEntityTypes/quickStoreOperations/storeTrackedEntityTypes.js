...
const convert = (() => {
  ...
  const getTrackedEntityTypeAttributes = (trackedEntityTypeAttributes) =>
    (trackedEntityTypeAttributes || []).map((trackedEntityTypeAttribute) =>
      getTrackedEntityTypeAttribute(trackedEntityTypeAttribute),
    );

  return (response) =>
    ((response && response.trackedEntityTypes) || []).map((trackedEntityType) => ({
      ...trackedEntityType,
      trackedEntityTypeAttributes: getTrackedEntityTypeAttributes(
...
const fieldsParam =
  'id,displayName,minAttributesRequiredToSearch,featureType,' +
