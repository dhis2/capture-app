...
function buildCacheCategoryOptions(categoryOptions: Array<Object>) {
...
async function loadCategoryOptionsBatchAsync(
  ...
  const optionsByCategory = addOptionsByCategory(
    categoryOptions,
    categoryIds,
    prevOptionsByCategory,
  );
...
async function setCategoriesAsync(categories: Array<Object>) {
  const { storageController, storeNames } = getContext();
 ...
 */
export async function loadCategories(uniqueCategories: Array<InputCategory>) {
