...
export const storeTrackedEntityAttributes = (ids: Array<string>) => {
    ...
    const converter = response => response && response.trackedEntityAttributes;

    return quickStore(query, getContext().storeNames.TRACKED_ENTITY_ATTRIBUTES, { onConvert: converter });
};
