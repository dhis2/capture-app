...
export const loadRulesCentricMetadata = (programIds: Array<string>) => {
    ...
    (
      await Promise.all(
        programIds.map((programId) =>
          storageController.getAll(storeName, {
            project: (item) => item.id,
            onIDBGetRequest: (source) =>
              source.index('programId').openCursor(window.IDBKeyRange.only(programId)),
          }),
        ),
      )
    ).flat(1);
  ...
  const removeUnavailableRecords = async (loadedIds, cachedIds, storeName) => {
    const loadedIdsAsObject = loadedIds.reduce((acc, ruleId) => {
      acc[ruleId] = ruleId;
      return acc;
    }, {});

    const unavailableIds = cachedIds.filter((cachedId) => !loadedIdsAsObject[cachedId]);
