...
const convert = (() => {
  ...
  const getIdProps = (apiProgramRuleVariable) => ({
    ...
    trackedEntityAttributeId:
      apiProgramRuleVariable.trackedEntityAttribute &&
  ...
  return (response) => {
    ...
    return apiProgramRulesVariables.map((apiProgramRuleVariable) => ({
      ...apiProgramRuleVariable,
      ...resetProps,
      ...getIdProps(apiProgramRuleVariable),
      useNameForOptionSet:
        apiProgramRuleVariable.useCodeForOptionSet != null
...
export const storeProgramRulesVariables = async (programIds: Array<string>) => {
  const query = {
    ...
    params: {
      fields:
        'id,displayName,programRuleVariableSourceType,' +
  ...
  const convertRetainingIds = (response) => {
    ...
    programRuleVariableIds = programRuleVariableIds.concat(
      convertedProgramRuleVariables.map((programRuleVariable) => programRuleVariable.id),
    );
