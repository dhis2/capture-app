...
const convert = (() => {
  ...
  const convertTranslationsToObject = (translations) =>
    (translations || []).reduce((accTranslationObject, translation) => {
  ...
  const getProgramStages = (apiProgramStages) => {
    const programStages = (apiProgramStages || []).map((apiProgramStage) => ({
      ...apiProgramStage,
      programStageDataElements: getProgramStageDataElements(
        apiProgramStage.programStageDataElements,
      ),
  ...
  const getOrganisationUnits = (apiOrganisationUnits) =>
    (apiOrganisationUnits || []).reduce((accOrganisationUnits, organisationUnit) => {
  ...
  const getProgramTrackedEntityAttribute = (programAttribute) => ({
    ...
    trackedEntityAttributeId:
      programAttribute.trackedEntityAttribute && programAttribute.trackedEntityAttribute.id,
  });

  const getProgramTrackedEntityAttributes = (programAttributes) =>
    (programAttributes || []).map((pa) => getProgramTrackedEntityAttribute(pa));

  return (response: apiProgramsResponse) => {
    const apiPrograms = (response && response.programs) || [];

    return apiPrograms.map((apiProgram) => ({
      ...apiProgram,
      ...
      programTrackedEntityAttributes: getProgramTrackedEntityAttributes(
        apiProgram.programTrackedEntityAttributes,
      ),
    }));
  };
})();
...
const fieldsParam =
  'id,version,displayName,displayShortName,description,programType,style,' +
