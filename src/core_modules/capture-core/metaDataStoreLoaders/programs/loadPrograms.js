...
import { storePrograms } from './quickStoreOperations';
...
const getCachedProgramsOutline = () => {
  ...
  return storageController.getAll(storeNames.PROGRAMS, {
    project: (program) => ({
      id: program.id,
...
const removeUnavailablePrograms = async (apiPrograms, cachedPrograms) => {
  const apiProgramsAsObject = apiPrograms.reduce((acc, apiProgram) => {
...
const getStaleProgramIds = (apiPrograms, cachedPrograms) => {
  const cachedProgramsAsObject = cachedPrograms.reduce((acc, cachedProgram) => {
...
const loadProgramBatch = async (programIds) => {
  ...
  return programs.map(({ programTrackedEntityAttributes, categoryCombo, trackedEntityTypeId }) => ({
...
const getSideEffects = (() => {
  const getOptionSetsOutline = (() => {
    const getDataElementOptionSets = (programStages) =>
      (programStages || []).flatMap((programStage) =>
    ...
    return (programsOutline): Array<Object> =>
      programsOutline.flatMap((program) => getProgramOptionSets(program));
  })();

  const getTrackedEntityAttributeIds = (stalePrograms) =>
    pipe(
      () =>
        stalePrograms.flatMap((program) =>
  ...
  const getCategories = (stalePrograms) =>
    pipe(
      () =>
        stalePrograms.flatMap(
          (program) => (program.categoryCombo && program.categoryCombo.categories) || [],
        ),
      (categories) => [...new Map(categories.map((ic) => [ic.id, ic])).values()],
    )();

  const getTrackedEntityTypes = (stalePrograms) =>
    pipe(
      () =>
        stalePrograms.reduce((acc, program) => {
...
export const loadPrograms = async () => {
  ...
  const programsDataForSideEffects: Array<Object> = (
    await Promise.all(programBatches.map(loadProgramBatch))
  ).flat(1);
