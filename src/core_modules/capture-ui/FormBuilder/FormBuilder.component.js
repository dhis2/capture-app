...
type FieldsValidatingPromiseContainer = {
  [fieldId: string]: ?{
    cancelableValidatingPromise?: ?CancelablePromise<any>,
...
class FormBuilder extends React.Component<Props> {
  static async validateField(
    ...
    const validatorResult = await field.validators.reduce(async (passPromise, currentValidator) => {
  ...
  static executeValidateAllFields(
    ...
    const { id, fields, values, onGetValidationContext, onIsValidating } = props;
    ...
    const validationPromises = fields.map(async (field) => {
      ...
      const handleIsValidatingInternal = (message: ?string, promise: Promise<any>) => {
        fieldValidatingPromiseContainer.cancelableValidatingPromise = makeCancelablePromise(
          promise,
        );
        onIsValidating &&
          onIsValidating(
    ...
    return (
      Promise.all(validationPromises)
        // $FlowFixMe
        .then(
          // $FlowFixMe[incompatible-cast] automated comment
          ((validationContainers) =>
            validationContainers.filter(
              (validationContainer) => !!validationContainer.validationData,
            ): Array<{ id: string, validationData: Object }>),
        )
    );
  }
  ...
  UNSAFE_componentWillReceiveProps(newProps: Props) {
    if (newProps.id !== this.props.id || newProps.loadNr !== this.props.loadNr) {
      ...
      this.asyncUIState = FormBuilder.updateAsyncUIState(
        this.props.fieldsUI,
        newProps.fieldsUI,
        this.asyncUIState,
      );
  ...
  getCleanUpData() {
    const remainingCompleteUids: Array<string> = Object.keys(this.fieldsValidatingPromiseContainer)
      .map((key) => {
  ...
  validateAllFields(props: Props) {
    ...
    this.validateAllCancelablePromise.promise
      .then((validationContainerArray) => {
        const validationContainers = validationContainerArray.reduce((accFieldsUI, container) => {
        ...
        const promisesAndIdForActuallyValidatedFields = validationContainerArray
          .map((validationDataContainer) => ({
            ...
            commitUpdateTriggeredForField: this.commitUpdateTriggeredForFields[
              validationDataContainer.id
            ],
          }))
        ...
        promisesAndIdForActuallyValidatedFields.forEach((container) => {
  ...
  isValid(typesToCheck?: ?Array<string>) {
    return Object.keys(this.props.fieldsUI).every((key) => {
  ...
  renderField = (
    ...
    return (
      <div key={field.id} className={defaultClasses.fieldOuterContainer}>
        <div {...(onGetContainerProps && onGetContainerProps(index, fields.length, field))}>
          {/* $FlowFixMe[cannot-spread-inexact] automated comment */}
          <field.component
            ref={(fieldInstance) => {
              this.setFieldInstance(fieldInstance, field.id);
            }}
            value={value}
