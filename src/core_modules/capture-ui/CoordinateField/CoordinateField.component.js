...
export default class D2Coordinate extends React.Component<Props, State> {
  ...
  onMapPositionChange = (mapCoordinates: any) => {
    ...
    this.setMapPosition(
      [this.toSixDecimal(lat), this.toSixDecimal(lng)],
      mapCoordinates.target.getZoom(),
    );
  };
    ...
    return (
      <div className={mapIconContainerClass}>
        <IconButton
          ...
          classes={{
            root: classNames(defaultClasses.mapIcon, mapIconCustomClass),
          }}
        >
          <AddLocationIcon onClick={this.openMap} />
        </IconButton>
      </div>
    );
  };
  ...
  search = (position: any) => {
    const zoom =
      this.mapInstance && this.mapInstance.leafletElement
  ...
  renderClearButton = () => {
    ...
    const clearIconClass = shrinkDisabled
      ? defaultClasses.clearIcon
      : defaultClasses.clearIconWithMargin;
    return (
  ...
  renderVertical = () => (
    <div>
      <div className={defaultClasses.coordinateFieldsVertical}>
        ...
        <div className={defaultClasses.inputContainer}>{this.renderLatitude()}</div>
        <div className={defaultClasses.inputContainer}>{this.renderLongitude()}</div>
      </div>
    </div>
  );
  ...
  render() {
    return this.props.orientation === orientations.VERTICAL
      ? this.renderVertical()
