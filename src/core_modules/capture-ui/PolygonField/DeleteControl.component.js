...
class DeleteControl extends MapControl<any, Props> {
  UNSAFE_componentWillMount() {
    ...
    const jsx = (
      <div className="leaflet-draw-toolbar leaflet-bar">
        ...
        <a
          className={classNames('leaflet-draw-edit-remove', {
            'leaflet-disabled': this.props.disabled,
          })}
  ...
  updateLeafletElement(fromProps: Props, toProps: Props): void {
    ...
    if (toProps.disabled !== fromProps.disabled) {
      const buttons = this.leafletElement
        .getContainer()
        .getElementsByClassName('leaflet-draw-edit-remove');
      if (buttons && buttons.length > 0) {
        buttons[0].className = classNames('leaflet-draw-edit-remove', {
          'leaflet-disabled': toProps.disabled,
        });
      }
    }
  }
