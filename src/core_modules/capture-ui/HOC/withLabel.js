...
export default (hocParams: ?HOCParamsContainer) => (InnerComponent: React.ComponentType<any>) =>
  class LabelHOC extends React.Component<Props> {
    ...
    UNSAFE_componentWillReceiveProps(newProps: Props) {
      ...
      if (Object.keys(nextClasses).length !== Object.keys(prevClasses).length) {
        ...
        Object.keys(nextClasses).some((nextKey) => nextClasses[nextKey] !== prevClasses[nextKey])
      ) {
        this.setClasses(nextClasses);
      }
    }
    ...
    render() {
      const { label, labelRef, classes, styles, ...passOnProps } = this.props;
      const useVerticalOrientation =
        hocParams &&
        hocParams.onGetUseVerticalOrientation &&
      ...
      const labelContainerClass = useVerticalOrientation
        ? defaultClasses.labelContainerVertical
        : defaultClasses.labelContainer;
      const stylesContainer = styles || {};
      return (
        <div
          className={
            useVerticalOrientation ? defaultClasses.containerVertical : defaultClasses.container
          }
        >
          <div
          ...
          >
            <InnerComponent {...this.passOnClasses} {...passOnProps} />
          </div>
        </div>
      );
    }
