...
class SingleSelectionBoxes extends React.Component<Props> {
  ...
  getCheckedClass = (iconSelected: ?string, iconDisabled?: string, isDisabled: ?boolean) =>
    classNames(iconSelected, iconDisabled && { [iconDisabled]: isDisabled });

  getUncheckedClass = (iconDeselected: ?string, iconDisabled?: string, isDisabled: ?boolean) =>
    classNames(iconDeselected, iconDisabled && { [iconDisabled]: isDisabled });

  getCheckedIcon() {
    const { classes, disabled } = this.props;
    const { iconDisabled, iconSelected } = classes || {};
    return <CheckedIcon className={this.getCheckedClass(iconSelected, iconDisabled, disabled)} />;
  }
  ...
  getUncheckedIcon() {
    ...
    return (
      <UncheckedIcon className={this.getUncheckedClass(iconDeselected, iconDisabled, disabled)} />
    );
  }
  ...
  getOption(optionData: OptionRendererInputData, isSelected: boolean, index: number) {
    ...
    const containerClass =
      orientation === orientations.HORIZONTAL
        ? defaultClasses.optionContainerHorizontal
        : defaultClasses.optionContainerVertical;
    const tabIndex =
      isSelected || (index === 0 && !value && value !== false && value !== 0) ? 0 : -1;
    const IconElement = this.getIconElement(optionData, isSelected);

    return (
      <div className={containerClass} key={optionData.id || optionData.name}>
        {/* $FlowSuppress */}
  ...
  render() {
    ...
    const containerClass =
      orientation === orientations.HORIZONTAL
        ? defaultClasses.containerHorizontal
        : defaultClasses.containerVertical;

    return <div className={containerClass}>{this.getSelectionOptions()}</div>;
  }
}
