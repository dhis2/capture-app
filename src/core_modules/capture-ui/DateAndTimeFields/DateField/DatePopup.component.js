...
class DatePopup extends React.Component<Props> {
  ...
  getAbsoluteVerticalPosition() {
    ...
    return absoluteDirection === absoluteDirections.UP
      ? { bottom: this.getAbsoluteBottom() }
      : { top: 0 };
  }
  ...
  getCenterCalendarStyle = (mode: ?$Values<typeof modes>) =>
    mode === modes.INLINE
      ...
      : {
          ...this.getAbsoluteVerticalPosition(),
          left: '50%',
          transform: 'translate(-50%, 0)',
        };

  render() {
    const { open, mode, children } = this.props;
    ...
    const containerClasses =
      mode === modes.INLINE ? defaultClasses.containerInline : defaultClasses.containerAbsolute;
    const calendarClasses =
      mode === modes.INLINE ? defaultClasses.calendarInline : defaultClasses.calendarAbsolute;
    const calendarStyle = this.getPopupStyle();

    return (
      <div className={containerClasses}>
        <div className={calendarClasses} style={calendarStyle}>
          {children}
        </div>
      </div>
    );
