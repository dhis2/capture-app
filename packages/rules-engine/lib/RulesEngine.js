    ...
    .flatMap(rule => {
      ...
      if (expression) {
        ...
        isProgramRuleExpressionEffective = executeExpression({
          ...
          dhisFunctions,
          variablesHash,
          onError: (error, injectedExpression) => log.warn(`Expression with id rule:${rule.id} could not be run. ` + `Original condition was: ${expression} - ` + `Evaluation ended up as:${injectedExpression} - error message:${error}`)
        });
      } else {
        log.warn(`Rule id:'${rule.id}' and name:'${rule.displayName}' ` + 'had no condition specified. Please check rule configuration.');
      }
        ...
        programRuleEffects = rule.programRuleActions.map(({
          ...
          if (actionExpression) {
            actionExpressionResult = executeExpression({
              ...
              dhisFunctions,
              variablesHash,
              onError: (error, injectedExpression) => log.warn(`Expression with id rule: action:${id} could not be run. ` + `Original condition was: ${actionExpression} - ` + `Evaluation ended up as:${injectedExpression} - error message:${error}`)
            });
          }
